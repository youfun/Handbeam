defmodule Handbeam.Agent.TurnHookPipelineTest do
  ...
  test "blocked tool produces denied result block in messages" do
    ...
    :ok = Handbeam.Extension.Registry.register(Handbeam.Extension.Registry, ext)

    HookPipeline.register_hook_module(
      Handbeam.Extension.Registry,
      "test-block-ext",
      BlockReadOnlyHook
    )
  ...
  test "active set excludes tool — blocked at execution side" do
    ...
    Handbeam.Tool.Registry.set_active_for_session(session_id, ["bash"])

    # Clean up active set after test
    on_exit(fn ->
      Handbeam.Tool.Registry.set_active_for_session(session_id, nil)
    end)
  ...
  defp build_extension(name, opts \\ []) do
    %Handbeam.Extension{
      name: name,
      root: "/abs/path/.handbeam/extensions/#{name}",
      enabled: true,
...
defmodule BeforeAgentStartBlockerHook do
  ...
  @impl true
  def handle_event(%Handbeam.Extension.Event{name: :before_agent_start}, _ctx),
...
defmodule ContextPromptInjectorHook do
  ...
  @impl true
  def handle_event(%Handbeam.Extension.Event{name: :context}, _ctx) do
...
defmodule ContextMessageRedactorHook do
  ...
  @impl true
  def handle_event(%Handbeam.Extension.Event{name: :context}, _ctx) do
    {:ok, %{messages: [Handbeam.Agent.Message.user("REDACTED_FOR_PROVIDER")]}}
  end
