defmodule Handbeam.Agent.ExtensionHookIntegrationTest do
  use ExUnit.Case, async: false

  alias Handbeam.Extension.Event
  alias Handbeam.Extension.HookRunner
  ...
  describe "Extension tool execution via Tool.Registry" do
    test "extension tool can be dispatched through Tool.Executor" do
      ...
      {:ok, result_msg, _ui_blocks} =
        Handbeam.Agent.Tool.Executor.execute_all_with_details(tool_calls, state)
    ...
    test "extension tool with input schema works" do
      :ok = Handbeam.Tool.Registry.register(TestExtensionTool2)
  ...
  defp build_test_state do
    ...
    Handbeam.Agent.State.init(config, "test prompt")
  end
end
