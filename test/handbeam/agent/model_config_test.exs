defmodule Handbeam.Agent.ModelConfigTest do
  @moduledoc """
  Tests for Handbeam.Agent.ModelConfig.
  ...
  setup do
    ...
    delete_config()
    System.put_env("HANDBEAM_MODELS_FILE", config_path())
  ...
  describe "workspace model policy" do
    ...
    test "no .handbeam/settings.jsonc returns all global models" do
      ws = tmp_workspace()
    ...
    test "malformed policy JSON returns error and no models" do
      ...
      policy_path = Handbeam.WorkspaceSettings.path(ws)
      File.mkdir_p!(Path.join(ws, ".handbeam"))
  ...
  describe "read_config/0" do
    ...
    test "returns the stored map untouched (no env override, no secret resolution)" do
      write_config(%{
        ...
        "defaultModel" => "m",
        "providers" => %{
          "p" => %{"apiKey" => "env:HANDBEAM_TEST_UNSET_KEY", "models" => [%{"id" => "m"}]}
        }
      })
      ...
      with_env("OPENAI_MODEL", "other", fn ->
        ...
        assert config["providers"]["p"]["apiKey"] == "env:HANDBEAM_TEST_UNSET_KEY"
      end)
    end
