defmodule Handbeam.Agent.TurnTest do
  @moduledoc """
  Tests for the core agent loop (Handbeam.Agent.Turn).
  ...
  defp ensure_registered(mod) do
    case Handbeam.Tool.Registry.get(mod.name()) do
      {:ok, _} -> :ok
      :error -> Handbeam.Tool.Registry.register(mod)
    end
  end
    ...
    def complete(_messages, _tools, _config) do
      {:ok,
       %{
         ...
         messages: [
           Handbeam.Agent.Message.tool_use([
             %{
               type: "tool_use",
               id: "sensitive",
               name: "read",
               input: %{"password" => "fixture-secret", "nested" => %{token: "nested-secret"}}
             }
           ])
         ],
  ...
  test "tool_start redacts secrets for every tool, not just browser" do
    ...
    owner = self()
    Turn.run_loop(State.init(config, "test"), on_event: &send(owner, &1))

    assert_receive {:tool_start, %{tool: "read", input: input}}
    assert input["password"] == "[REDACTED]"
    assert input["nested"][:token] == "[REDACTED]"
  end
  ...
  describe "simple completion (end_turn)" do
    ...
    test "continues with queued follow_up after a provider timeout" do
      ...
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(session_id: "turn-timeout-follow", owner: self())

      :ok = Handbeam.Agent.CandidateQueue.enqueue(queue, "继续", deliver_as: :follow_up)
  ...
  describe "run_loop/1 with :after_compaction hook" do
    test "fires :after_compaction when compaction occurs" do
      ...
      state =
        State.init(config, [
          Handbeam.Agent.Message.user("original"),
          Handbeam.Agent.Message.assistant(String.duplicate("a", 400)),
          Handbeam.Agent.Message.user("latest")
        ])
    ...
    test "does NOT fire :after_compaction when within budget" do
      ...
      state = State.init(config, [Handbeam.Agent.Message.user("Hi")])
      Turn.run_loop(state)
  ...
  describe "candidate queue injection" do
    test "injects steer messages before provider call" do
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(session_id: "turn-steer", owner: self())

      :ok =
        Handbeam.Agent.CandidateQueue.enqueue(queue, "steer now",
          deliver_as: :steer,
    ...
    test "injects steer messages after tool execution before the next provider call" do
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(session_id: "turn-tool-steer", owner: self())
        ...
        Turn.run_loop(state,
          ...
          on_event: fn
            {:tool_end, _payload} = event ->
              ...
              Handbeam.Agent.CandidateQueue.enqueue(queue, "after tool",
                deliver_as: :steer,
    ...
    test "continues after end_turn when follow_up messages are pending" do
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(session_id: "turn-follow", owner: self())

      :ok =
        Handbeam.Agent.CandidateQueue.enqueue(queue, "follow up",
    ...
    test "emits message_ids for remaining follow_up after start-of-turn steer drain" do
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(session_id: "turn-both-pending", owner: self())

      :ok =
        Handbeam.Agent.CandidateQueue.enqueue(queue, "steer first",
          deliver_as: :steer,
          message_id: "msg-both-steer"
        )

      :ok =
        Handbeam.Agent.CandidateQueue.enqueue(queue, "follow later",
          deliver_as: :follow_up,
    ...
    test "keeps completed behavior when no pending messages exist" do
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(session_id: "turn-none", owner: self())
      ...
      assert {:error, :sealed} =
               Handbeam.Agent.CandidateQueue.enqueue(queue, "arrived after completion")
    end
...
defmodule Handbeam.Agent.TurnPermissionTest do
  use ExUnit.Case, async: true
  ...
  test "deny policy injects steer candidates after blocked tools" do
    ...
    defmodule EnqueueSteerAfterToolRequest do
      ...
      def call(:after_tool_request, state) do
        ...
        :ok =
          Handbeam.Agent.CandidateQueue.enqueue(queue, "steer after deny-block",
            deliver_as: :steer,
    ...
    config = %Config{
      ...
      max_turns: 5,
      working_directory: workspace,
      middleware: [EnqueueSteerAfterToolRequest, Handbeam.Agent.Middleware.ToolGuard],
      context: %{queue: queue},
      provider_config: %{}
    }
  ...
  describe "resume_after_tool_approval/3" do
    ...
    test "all-denied resume injects steer candidates with message_ids" do
      {:ok, queue} =
        Handbeam.Agent.CandidateQueue.start_link(
          session_id: "turn-resume-deny-steer",
          owner: self()
        )

      :ok =
        Handbeam.Agent.CandidateQueue.enqueue(queue, "steer after deny",
          deliver_as: :steer,
    ...
    test "approve resume injects steer candidates after the HITL tool executes" do
      ...
      :ok =
        Handbeam.Agent.CandidateQueue.enqueue(queue, "steer after approve",
          deliver_as: :steer,
