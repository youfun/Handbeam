defmodule Handbeam.Agent.Provider.StepFunTest do
  use ExUnit.Case, async: false

  alias Handbeam.Agent.Message
  alias Handbeam.Agent.Provider.StepFun
  ...
  defmodule StreamingTextMockReq do
    def request(opts) do
      Handbeam.Agent.Provider.StepFunTest.record_stream_request(opts)

      Handbeam.Agent.Provider.StepFunTest.feed_sse(opts[:into], [
        Handbeam.Agent.Provider.StepFunTest.openai_delta(%{"content" => "Hello"}),
        Handbeam.Agent.Provider.StepFunTest.openai_delta(%{"content" => " World"}, "stop"),
        Handbeam.Agent.Provider.StepFunTest.openai_usage(10, 5),
        "data: [DONE]\n\n"
      ])
    end
  end
  ...
  defmodule StreamingToolUseMockReq do
    def request(opts) do
      ...
      Handbeam.Agent.Provider.StepFunTest.feed_sse(opts[:into], [
        ...
        Handbeam.Agent.Provider.StepFunTest.openai_delta(%{}, "tool_calls"),
        Handbeam.Agent.Provider.StepFunTest.openai_usage(20, 15),
        "data: [DONE]\n\n"
      ])
    end
  end
  ...
  defmodule StreamingTextWithToolMockReq do
    def request(opts) do
      ...
      Handbeam.Agent.Provider.StepFunTest.feed_sse(opts[:into], [
        Handbeam.Agent.Provider.StepFunTest.openai_delta(%{"content" => "I will read the file"}),
        Handbeam.Agent.Provider.StepFunTest.openai_delta(%{
          "tool_calls" => [
            %{
              "index" => 0,
        ...
        Handbeam.Agent.Provider.StepFunTest.openai_delta(%{}, "tool_calls"),
        Handbeam.Agent.Provider.StepFunTest.openai_usage(25, 20),
        "data: [DONE]\n\n"
      ])
    end
  end
  ...
  test "Handbeam.Agent.Message.text/1 works with content blocks" do
    msg = %Message{role: :assistant, content: [%{type: "text", text: "Hello"}]}
    assert Message.text(msg) == "Hello"
  end

  test "Handbeam.Agent.Message.tool_calls/1 extracts tool_use blocks" do
    msg = %Message{
      role: :assistant,
      content: [
