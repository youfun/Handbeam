defmodule Handbeam.Runtime.AndroidNotifyTest do
  use ExUnit.Case, async: false

  alias Handbeam.Runtime.AndroidNotify
  ...
  setup do
    previous = Application.get_env(:handbeam, :notifier)

    on_exit(fn ->
      if previous,
        do: Application.put_env(:handbeam, :notifier, previous),
        else: Application.delete_env(:handbeam, :notifier)
    end)
  ...
  test "without a notifier every action is a no-op and the app is not visible" do
    Application.delete_env(:handbeam, :notifier)
  ...
  test "a fun/2 notifier receives the NIF-style call name and encoded JSON" do
    ...
    Application.put_env(:handbeam, :notifier, fn fun, args ->
  ...
  test "a module notifier is called by function name" do
    ...
    Application.put_env(:handbeam, :notifier, NotifierStub)
  ...
  test "a notifier module missing the function degrades to the no-op fallback" do
    Application.put_env(:handbeam, :notifier, Handbeam.Runtime.AndroidNotifyTest.MissingNotifier)
