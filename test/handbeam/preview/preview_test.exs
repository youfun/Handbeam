defmodule Handbeam.PreviewTest do
  use ExUnit.Case, async: false

  alias Handbeam.Preview
  alias Handbeam.Preview.{Files, Proxy, URL}
  alias Handbeam.Tool.Builtin.PreviewServe

  setup do
    root = Path.join(System.tmp_dir!(), "handbeam-preview-#{System.unique_integer([:positive])}")
  ...
  test "preview_serve registers files and returns a card payload", %{root: root} do
    ...
    assert details.return_href == "handbeam://c/conv-card"
  end
  ...
  test "shell URL matcher only accepts loopback PreviewShell paths" do
    ...
    assert {:ok, "abc"} = URL.return_conversation_id("handbeam://c/abc")
    assert :error = URL.return_conversation_id("https://evil.example/handbeam://c/abc")
  end
end
