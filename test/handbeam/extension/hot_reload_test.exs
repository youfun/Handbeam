defmodule Handbeam.Extension.HotReloadTest do
  @moduledoc """
  ...
  alias Handbeam.Agent.ExtensionBridge
  alias Handbeam.Extension.HotReloader
  alias Handbeam.Extension.Registry, as: ExtRegistry
  ...
  setup do
    ...
    on_exit(fn ->
      if Process.whereis(Handbeam.Extension.Supervisor) do
        Handbeam.Extension.Supervisor.replace_workers("hotdemo", [])
      end
    end)
  ...
  test "reload swaps a compiled entry module and reregisters its tool", %{
    ...
    assert {:ok, entry} = Handbeam.Tool.Registry.get("ext__hotdemo__ping")
    ...
    assert {:ok, new_entry} = Handbeam.Tool.Registry.get("ext__hotdemo__ping")
  ...
  test "reload overrides tool description in the registry", %{tmp: tmp, registry: registry} do
    ...
    assert {:ok, updated} = Handbeam.Tool.Registry.get("ext__hotdemo__ping")
  ...
  test "reload keeps the worker pid when only module code changes", %{
    ...
    workers = Handbeam.Extension.Supervisor.workers("hotdemo")
    ...
    assert Handbeam.Extension.Supervisor.workers("hotdemo") == [pid]
    assert Process.alive?(pid)
    assert Ext.Hotdemo.Worker.started_as() == :v1
  ...
  test "reload does not drop a running worker when the next compile fails", %{
    ...
    pid = hd(Handbeam.Extension.Supervisor.workers("hotdemo"))
    assert {:ok, entry} = Handbeam.Tool.Registry.get("ext__hotdemo__ping")
    assert {:ok, ":v1"} = entry.executor.(%{}, %{})

    File.write!(
      Path.join(tmp, ".handbeam/extensions/hotdemo/hot_combo.ex"),
    ...
    assert Process.alive?(pid)
    assert Handbeam.Extension.Supervisor.workers("hotdemo") == [pid]
    assert {:ok, still} = Handbeam.Tool.Registry.get("ext__hotdemo__ping")
  ...
  test "successful reload removes stale tools and hooks", %{tmp: tmp, registry: registry} do
    ...
    assert {:ok, _} = Handbeam.Tool.Registry.get("ext__hotdemo__ping")
    ...
    assert Handbeam.Tool.Registry.get("ext__hotdemo__ping") == :error
  ...
  test "deleting an extension directory removes its runtime resources", %{
    ...
    assert {:error, :not_found} = ExtRegistry.get(registry, "hotdemo")
    assert Handbeam.Tool.Registry.get("ext__hotdemo__ping") == :error
    assert Handbeam.Extension.Supervisor.workers("hotdemo") == []
  ...
  test "notify_path reloads an extension outside the server cwd", %{
    ...
    HotReloader.notify_path(
      Path.join(other, ".handbeam/extensions/hotdemo/extension.json"),
      pid
    )
  ...
  defp write_entry_extension!(tmp, name, opts) do
    ...
    ext_dir = Path.join(tmp, ".handbeam/extensions/#{name}")
