defmodule Handbeam.ExtensionTest do
  use Handbeam.DataCase, async: true

  alias Handbeam.Extension
  ...
  describe "new/2 - build from manifest" do
    test "builds extension struct with all fields" do
      manifest = %Manifest{
        name: "agent-notify",
        version: "0.1.0",
        description: "Post Handbeam lifecycle events to AgentNotify.",
        entry: "extension.exs",
        enabled: true,
        permissions: %{
      ...
      root = "/abs/path/.handbeam/extensions/agent-notify"

      assert {:ok, ext} = Extension.new(manifest, root)
      assert ext.name == "agent-notify"
      assert ext.version == "0.1.0"
      assert ext.description == "Post Handbeam lifecycle events to AgentNotify."
      assert ext.root == root
      assert ext.entry == "/abs/path/.handbeam/extensions/agent-notify/extension.exs"
    ...
    test "entry defaults to nil when not in manifest" do
      manifest = %Manifest{name: "test-ext"}
      root = "/abs/path/.handbeam/extensions/test-ext"
    ...
    test "entry is resolved relative to root" do
      ...
      assert {:ok, ext} = Extension.new(manifest, root)
      assert ext.entry == "/abs/path/.handbeam/extensions/test-ext/lib/runner.exs"
    end
  ...
  describe "active?/1" do
    test "enabled extension is active" do
      ...
      {:ok, ext} = Extension.new(manifest, "/abs/path/.handbeam/extensions/test-ext")
