defmodule Handbeam.Extension.MountTest do
  @moduledoc """
  ...
  alias Handbeam.Extension.Mount
  ...
  test "mount registers a compiled tool from source and execute works", %{n: n} do
    ...
    assert {:ok, entry} = Handbeam.Tool.Registry.get("ext__mem#{n}__ping")
  ...
  test "replacing the same id keeps the new tool registered", %{n: n} do
    ...
    assert {:ok, first} = Handbeam.Tool.Registry.get("ext__mem#{n}__ping")
    assert {:ok, "v1"} = first.executor.(%{}, %{})

    assert {:ok, _} = Mount.mount(ping_source(n, "v2"), name: "mem#{n}")
    assert {:ok, second} = Handbeam.Tool.Registry.get("ext__mem#{n}__ping")
  ...
  test "unmount removes the tool and stops the worker", %{n: n} do
    ...
    workers = Handbeam.Extension.Supervisor.workers("mem#{n}")
    assert length(workers) == 1
    pid = hd(workers)
    assert Process.alive?(pid)
    assert {:ok, _} = Handbeam.Tool.Registry.get("ext__mem#{n}__ping")

    assert :ok = Mount.unmount("mem#{n}")

    refute Process.alive?(pid)
    assert Handbeam.Extension.Supervisor.workers("mem#{n}") == []
    assert Handbeam.Tool.Registry.get("ext__mem#{n}__ping") == :error
  end
