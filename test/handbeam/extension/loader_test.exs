defmodule Handbeam.Extension.LoaderTest do
  use ExUnit.Case, async: true

  alias Handbeam.Extension.Loader
  ...
  describe "from_project/1" do
    test "discovers project extensions from .handbeam/extensions", %{tmp: tmp} do
      ext_dir = Path.join(tmp, ".handbeam/extensions/my-ext")
    ...
    test "returns empty when no .handbeam/extensions directory", %{tmp: tmp} do
      result = Loader.from_project(tmp)
      assert result.extensions == []
    end

    test "returns empty when directory exists but is empty", %{tmp: tmp} do
      sigil_ext = Path.join(tmp, ".handbeam/extensions")
    ...
    test "discovers nested extension directories", %{tmp: tmp} do
      nested = Path.join(tmp, ".handbeam/extensions/group/my-ext")
    ...
    test "skips hidden directories", %{tmp: tmp} do
      hidden = Path.join(tmp, ".handbeam/extensions/.hidden-ext")
    ...
    test "skips node_modules", %{tmp: tmp} do
      nm = Path.join(tmp, ".handbeam/extensions/node_modules/some-ext")
    ...
    test "handles invalid JSON with diagnostic", %{tmp: tmp} do
      bad = Path.join(tmp, ".handbeam/extensions/bad-ext")
    ...
    test "handles duplicate names (first wins)", %{tmp: tmp} do
      ext1 = Path.join(tmp, ".handbeam/extensions/my-ext")
      ext2 = Path.join(tmp, ".handbeam/extensions/deeper/my-ext")
    ...
    test "disabled extension is loaded but not active", %{tmp: tmp} do
      ext_dir = Path.join(tmp, ".handbeam/extensions/disabled-ext")
  ...
  describe "from_user/2" do
    test "discovers user extensions from custom home", %{tmp: tmp} do
      ...
      ext_dir = Path.join(home, ".handbeam/extensions/user-ext")
    ...
    test "returns empty when user home has no extensions", %{tmp: tmp} do
      ...
      File.mkdir_p!(Path.join(home, ".handbeam/extensions"))
  ...
  describe "load/1 - combined" do
    test "project wins over user for duplicate names", %{tmp: tmp} do
      ...
      user_ext = Path.join(home, ".handbeam/extensions/shared-ext")
      write_manifest(user_ext, "shared-ext", %{"version" => "0.1.0-user"})

      # Project
      proj_ext = Path.join(tmp, ".handbeam/extensions/shared-ext")
    ...
    test "user extensions loaded when no project overlap", %{tmp: tmp} do
      home = Path.join(tmp, "user_home")
      user_ext = Path.join(home, ".handbeam/extensions/user-only")
    ...
    test "combines project and user extensions", %{tmp: tmp} do
      home = Path.join(tmp, "user_home")
      write_manifest(Path.join(home, ".handbeam/extensions/ext-user"), "ext-user")
      write_manifest(Path.join(tmp, ".handbeam/extensions/ext-proj"), "ext-proj")
