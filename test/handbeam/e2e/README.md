...
# 跑特定 e2e 测试文件
mix test --include e2e test/handbeam/e2e/
mix test --include e2e test/handbeam_web/live/
...
### Agent Core e2e (`test/handbeam/e2e/fake_provider_e2e_test.exs`)
...
### LiveView smoke (`test/handbeam_web/live/workspace_live_test.exs`)
...
| 测试类型 | 建议位置 | 标签 |
|----------|----------|------|
| 100k tokens 流式压测 | `test/handbeam/e2e/streaming_stress_test.exs` | `@slow` |
| 真实 Anthropic API 测试 | `test/handbeam/e2e/external_api_test.exs` | `@external_api` |
| Memory 集成测试（修复 sandbox） | 当前 `fake_provider_e2e_test.exs` memory 测试取消 skip | `@e2e` |
| 浏览器 E2E（Playwright/Wallaby） | `test/handbeam/e2e/browser/` | `@e2e @slow` |

## 真实 API 测试（external_api）

真实 API 测试目前只有一个 manual gate。如需自动测试，在 `test/handbeam/e2e/external_api_test.exs` 中创建测试文件，使用 `@moduletag :external_api`，运行时提供 `ANTHROPIC_API_KEY` 环境变量：

```elixir
defmodule Handbeam.E2E.ExternalApiTest do
  ...
  test "real Anthropic turn" do
    ...
    {:ok, result} = Handbeam.Agent.Provider.Anthropic.complete(
      [Handbeam.Agent.Message.user("Say hi")],
      [],
      config
    )
