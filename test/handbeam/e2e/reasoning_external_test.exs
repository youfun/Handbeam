defmodule Handbeam.E2E.ReasoningExternalTest do
  @moduledoc """
  Real provider smoke test for models.json reasoning configuration.

  Run explicitly:

      mix test test/handbeam/e2e/reasoning_external_test.exs --include external_api
  """
  ...
  alias Handbeam.Agent.{Message, ModelConfig, Reasoning}
  ...
  test "current reasoning-capable models.json model returns a real response" do
    ...
    provider =
      Handbeam.Agent.Config.resolve_provider_from_api(
        provider_config[:api],
        model_id,
