defmodule Handbeam.E2E.OpenAICompatibleExternalTest do
  @moduledoc """
  External smoke tests for a real OpenAI-compatible `/v1/chat/completions` endpoint.

  Defaults target StepFun Step Plan:

      OPENAI_API_KEY=... mix test test/handbeam/e2e/openai_compatible_external_test.exs --include external_api
  ...
  alias Handbeam.Agent.Provider.OpenAICompatible
  ...
  describe "real OpenAI-compatible provider" do
    test "completes a simple non-tool turn" do
      ...
      {result, _log} =
        with_log(fn ->
          OpenAICompatible.complete(
            [Handbeam.Agent.Message.user("Reply with exactly: handbeam-stepfun-ok")],
            [],
            config
          )
        end)
      ...
      assert assistant.content =~ "handbeam-stepfun-ok"
    end
