defmodule Handbeam.SettingsTest do
  use ExUnit.Case, async: true

  alias Handbeam.Settings
  alias Handbeam.Settings.ModelAISettings

  @tmp_base Path.join(System.tmp_dir!(), "handbeam_settings_test_#{System.unique_integer()}")
  ...
  describe "workspace model_ai settings" do
    ...
    test "load_workspace_model_ai returns model_ai section from settings.jsonc", %{ws_dir: ws_dir} do
      sigil_dir = Path.join(ws_dir, ".handbeam")
    ...
    test "save_workspace_model_ai creates .handbeam directory if missing" do
      ws_root = Path.join(@tmp_base, "fresh_ws")

      assert :ok =
               Settings.save_workspace_model_ai(ws_root, %{"default_model" => "claude-sonnet-4"})

      sigil_dir = Path.join(ws_root, ".handbeam")
