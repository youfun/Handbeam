defmodule Handbeam.ConversationStoreTest do
  ...
  describe "pin/1 and unpin/1" do
    test "pin survives rename and a later upsert that omits the field" do
      {:ok, conversation} = ConversationStore.create("ws_1", title: "Pinnable")
      refute ConversationStore.pinned_conversation?(conversation)

      assert {:ok, pinned} = ConversationStore.pin(conversation["id"])
      assert is_binary(pinned["pinned_at"])

      index_json = File.read!(ConversationStore.index_path()) |> Jason.decode!()
      [idx_entry] = index_json["conversations"]
      assert idx_entry["pinned_at"] == pinned["pinned_at"]

      assert {:ok, _} = ConversationStore.rename(conversation["id"], "Still pinned")
      {:ok, renamed} = ConversationStore.get_meta(conversation["id"])
      assert renamed["title"] == "Still pinned"
      assert renamed["pinned_at"] == pinned["pinned_at"]

      assert {:ok, saved} =
               conversation
               |> Map.delete("pinned_at")
               |> Map.put("title", "upserted")
               |> ConversationStore.upsert()

      assert saved["pinned_at"] == pinned["pinned_at"]

      assert {:ok, unpinned} = ConversationStore.unpin(conversation["id"])
      assert is_nil(unpinned["pinned_at"])
      refute ConversationStore.pinned_conversation?(unpinned)
    end
  end
