defmodule Handbeam.Memory.MemoryStoreTest do
  @moduledoc """
  ...
  Uses Handbeam.DataCase for DB sandbox isolation.
  """

  use Handbeam.DataCase

  alias Handbeam.Memory.{Engram, MemoryStore}
  ...
  describe "cleanup_expired/0 — expired engram deletion" do
    test "delete expired short-term engrams" do
      ...
      {:ok, expired_engram} = Handbeam.Repo.insert(expired)
      ...
      assert Handbeam.Repo.get(Engram, expired_engram.id) == nil

      # Verify active is still there
      assert Handbeam.Repo.get(Engram, active.id) != nil
    end
    ...
    test "does not delete long-term engrams" do
      ...
      assert Handbeam.Repo.get(Engram, lt.id) != nil
    end
  end
end
