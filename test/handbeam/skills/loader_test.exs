defmodule Handbeam.Skills.LoaderTest do
  @moduledoc """
  Tests for the Skills Loader.

  Covers:
    - Loading SKILL.md from explicit skill dir
    - Recursive discovery of SKILL.md in workspace .handbeam/skills
  ...
  describe "duplicate name" do
    test "first skill of duplicate name retained, second gets collision diagnostic" do
      skills_dir = Path.join(@tmp_base, ".handbeam/skills")
  ...
  describe "load/1" do
    test "loads from workspace .handbeam/skills directory" do
      workspace = Path.join(@tmp_base, "ws")
      skills_dir = Path.join(workspace, ".handbeam/skills")
    ...
    test "loads global skills from ~/.handbeam/skills and ~/.agents/skills" do
      workspace = Path.join(@tmp_base, "ws-global")
      home = Path.join(@tmp_base, "home-global")

      sigil_skill_dir = Path.join(home, ".handbeam/skills/global-sigil")
      agents_skill_dir = Path.join(home, ".agents/skills/global-agents")

      write_skill(sigil_skill_dir, "global-sigil", "Global Handbeam skill")
    ...
    test "project skill wins over global skill with same name" do
      ...
      write_skill(
        Path.join(workspace, ".handbeam/skills/shared-skill"),
        "shared-skill",
        "Project description"
      )

      write_skill(
        Path.join(home, ".handbeam/skills/shared-skill"),
        "shared-skill",
        "Global description"
      )
