defmodule Handbeam.MCP.ConfigLoaderTest do
  @moduledoc """
  ...
  describe "parse" do
    ...
    test "parses .handbeam/mcp.json in project" do
      project = Path.join(@tmp_base, "proj2")

      write_json(Path.join(project, ".handbeam"), "mcp.json", %{
  ...
  describe "user + project merge" do
    test "loads default user config from ~/.handbeam/mcp.json", %{tmp: tmp} do
      home = Path.join(tmp, "home")
      project_dir = Path.join(tmp, "default-user-proj")

      write_json(Path.join(home, ".handbeam"), "mcp.json", %{
  ...
  describe "env:VAR" do
    test "resolves env:VAR to runtime config" do
      System.put_env("HANDBEAM_TEST_TOKEN", "secret-value")
      ...
      write_json(project, ".mcp.json", %{
        "mcpServers" => %{
          "api" => %{
            "command" => "node",
            "env" => %{"TOKEN" => "env:HANDBEAM_TEST_TOKEN", "PLAIN" => "static"}
          }
        }
      })
      ...
      System.delete_env("HANDBEAM_TEST_TOKEN")
    end
  ...
  describe "diagnostic safety" do
    ...
    test "diagnostics do not leak env values" do
      System.put_env("HANDBEAM_SECRET", "do-not-leak")
      project = Path.join(@tmp_base, "secret")

      write_json(project, ".mcp.json", %{
        "mcpServers" => %{"s" => %{"command" => "n", "env" => %{"K" => "env:HANDBEAM_SECRET"}}}
      })
      ...
      System.delete_env("HANDBEAM_SECRET")
    end
  end
  ...
  describe ".mcp.json + .handbeam/mcp.json coexistence" do
    test ".handbeam/mcp.json overrides .mcp.json for same server" do
      ...
      write_json(Path.join(project, ".handbeam"), "mcp.json", %{
        "mcpServers" => %{
          "shared" => %{"command" => "from-handbeam-json", "args" => ["b"]},
      ...
      assert config.servers["shared"].command == "from-handbeam-json"
  ...
  describe "http / sse server configs" do
    ...
    test "url server with env:VAR in headers resolves correctly" do
      System.put_env("HANDBEAM_HTTP_TOKEN", "http-token-value")
      ...
      write_json(project, ".mcp.json", %{
        "mcpServers" => %{
          "remote" => %{
            ...
            "headers" => %{"Authorization" => "Bearer env:HANDBEAM_HTTP_TOKEN"}
          }
        }
      })
      ...
      assert config.servers["remote"].headers["Authorization"] == "Bearer env:HANDBEAM_HTTP_TOKEN"
    ...
    test "http server diagnostics do not leak header values" do
      System.put_env("HANDBEAM_HTTP_SECRET", "do-not-leak")
      ...
      write_json(project, ".mcp.json", %{
        "mcpServers" => %{
          "s" => %{
            "url" => "http://localhost:4000/mcp",
            "headers" => %{"Authorization" => "Bearer env:HANDBEAM_HTTP_SECRET"}
          }
        }
      })
      ...
      assert config.servers["s"].headers["Authorization"] == "Bearer env:HANDBEAM_HTTP_SECRET"
      System.delete_env("HANDBEAM_HTTP_SECRET")
    end
  end
  ...
  describe "mcpServers edge cases" do
    ...
    test "diagnostics never include runtime_env values" do
      System.put_env("HANDBEAM_RT", "sensitive-rt")
      project = Path.join(@tmp_base, "rt")

      write_json(project, ".mcp.json", %{
        "mcpServers" => %{"s" => %{"command" => "n", "env" => %{"K" => "env:HANDBEAM_RT"}}}
      })
