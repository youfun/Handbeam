defmodule Handbeam.WorkspaceTest do
  @moduledoc """
  Tests for Handbeam.Workspace — workspace path management.

  Covers:
    - Default root is ~/.handbeam/workspace
    - HANDBEAM_WORKSPACE env var overrides default
  ...
  alias Handbeam.Workspace

  @default_ws Path.expand("~/.handbeam/workspace")
  @test_ws Path.join(System.tmp_dir!(), "sigil_ws_test_#{System.unique_integer([:positive])}")

  setup do
    # Save current env/application state
    saved_env = System.get_env("HANDBEAM_WORKSPACE")
    saved_app = Application.get_env(:handbeam, :workspace_root)

    # Clean up for isolated tests
    System.delete_env("HANDBEAM_WORKSPACE")
    Application.delete_env(:handbeam, :workspace_root)

    on_exit(fn ->
      # Restore
      if saved_env, do: System.put_env("HANDBEAM_WORKSPACE", saved_env)
      if saved_app, do: Application.put_env(:handbeam, :workspace_root, saved_app)
  ...
  describe "root/0" do
    test "default root is ~/.handbeam/workspace" do
      assert Workspace.root() == @default_ws
    end

    test "HANDBEAM_WORKSPACE env var overrides default" do
      System.put_env("HANDBEAM_WORKSPACE", @test_ws)
      assert Workspace.root() == Path.expand(@test_ws)
    end

    test "application env overrides default" do
      Application.put_env(:handbeam, :workspace_root, @test_ws)
      assert Workspace.root() == Path.expand(@test_ws)
    end
    ...
    test "env var has priority over application env" do
      ...
      System.put_env("HANDBEAM_WORKSPACE", env_path)
      Application.put_env(:handbeam, :workspace_root, app_path)

      assert Workspace.root() == Path.expand(env_path)
    end

    test "expands tilde paths" do
      System.put_env("HANDBEAM_WORKSPACE", "~/custom/ws")
  ...
  describe "ensure_root!/0" do
    test "creates directory if it doesn't exist" do
      ...
      System.put_env("HANDBEAM_WORKSPACE", ws)
