...
class HandbeamBridgeTest {
    ...
    fun tapHandleForFindsNestedIdAndCoercesHandle() {
        ...
        assertEquals(42, HandbeamBridge.tapHandleFor(root, BridgeIds.Elements.DISMISS_APPROVAL))
        assertEquals("button", HandbeamBridge.findById(root, "dismiss_approval")?.type)
    }

    @Test
    fun tapHandleForIsNullWhenMissing() {
        val root = node("box", emptyMap(), node("text", mapOf("id" to "x")))
        assertNull(HandbeamBridge.tapHandleFor(root, "missing"))
        assertNull(HandbeamBridge.tapHandleFor(root, null))
        assertNull(HandbeamBridge.tapHandleFor(null, "x"))
        // id matches but no on_tap
        assertNull(HandbeamBridge.tapHandleFor(root, "x"))
    }
