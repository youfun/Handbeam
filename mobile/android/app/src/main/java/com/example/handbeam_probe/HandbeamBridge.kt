...
import com.example.handbeam_probe.attachments.LocalImagePreview
import com.example.handbeam_probe.attachments.PlatformHost
import com.example.handbeam_probe.attachments.StagingRoots
import com.example.handbeam_probe.workspace.NativeFileViewer
import java.util.concurrent.Executor
import java.util.concurrent.Executors
import java.util.concurrent.atomic.AtomicBoolean

/**
 * Handbeam Probe product hooks that used to live inside the `mob_new` scaffold
 ...
 * (`Java_com_example_handbeam_1probe_MobBridge_*`, and `platformCommand` via
 * `GetStaticMethodID` on the `MobBridge` class in `c_src/handbeam_browser.c`)
 * must remain declared on [MobBridge]. Those declarations are one-line
 * delegations into this object.
 */
object HandbeamBridge {
    private const val TAG = "HandbeamBridge"
    ...
    fun handleDirectoryResult(uri: Uri?) {
        ...
        }, "handbeam-workspace-import").start()
    }
...
internal class SerialCommandDispatcher(
    private val executor: Executor = Executors.newSingleThreadExecutor { runnable ->
        Thread(runnable, "handbeam-platform-command")
    },
    private val onError: (Throwable) -> Unit = { Log.e("HandbeamBridge", "platformCommand failed", it) },
) {
 ...
 */
internal object HandbeamRender {
    ...
    fun renderShell(node: MobNode, m: Modifier): Boolean {
        if (boolProp(node.props, BridgeIds.Props.APPROVAL_DIALOG) == true) {
            Dialog(onDismissRequest = {
                HandbeamBridge.sendTapTo(node, BridgeIds.Elements.DISMISS_APPROVAL)
            }) {
        ...
        if (boolProp(node.props, BridgeIds.Props.HISTORY_SHELL) == true) {
            NativeHistoryDrawer(
                ...
                onDismiss = {
                    HandbeamBridge.sendTapTo(node, node.props[BridgeIds.Props.BACK_TARGET] as? String)
                },
                modifier = m,
     ...
     */
    @Composable
    fun renderCustomNode(node: MobNode, bare: Modifier, trackId: String?) {
        val tracked = if (trackId != null) bare.then(MobBridge.frameTrackingModifier(trackId)) else bare
        when (node.type) {
            BridgeIds.Types.SETTINGS_SELECT -> SettingsSelect(node, tracked)
    ...
    fun renderPlainTextField(node: MobNode, modifier: Modifier): Boolean {
        ...
        HandbeamTextField(node, modifier)
        return true
    }
