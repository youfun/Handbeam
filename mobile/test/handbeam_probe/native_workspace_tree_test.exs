defmodule HandbeamProbe.NativeWorkspaceTreeTest do
  ...
  alias HandbeamProbe.NativeWorkspaceTree

  setup do
    Application.put_env(:handbeam_probe, :file_tree_max_entries, 2)

    on_exit(fn ->
      Application.delete_env(:handbeam_probe, :file_tree_max_entries)
      Application.delete_env(:handbeam_probe, :workspace_files_list)
      Application.delete_env(:handbeam_probe, :workspace_io_start)
    end)
  ...
  test "out-of-order directory results keep both listings" do
    ...
    Application.put_env(:handbeam_probe, :workspace_files_list, fn ws, dir, opts ->
  ...
  test "start_child failure clears loading and retry reloads a missing folder" do
    root = tree_root()
    Application.put_env(:handbeam_probe, :workspace_io_start, fn _fun -> {:error, :noproc} end)
    ...
    Application.put_env(:handbeam_probe, :file_tree_max_entries, 256)
    socket = NativeWorkspaceTree.handle(socket, bound(socket, %{op: :refresh}))
    socket = flush_listed(socket, 1)
  ...
  defp ensure_tree(root) do
    Mob.Socket.new(HandbeamProbe.HomeScreen)
    |> assign(:workspace, %{"id" => "ws-1", "path" => root, "name" => "W"})
    |> assign(:workspace_tree, NativeWorkspaceTree.idle())
    |> assign(:file_viewer, HandbeamProbe.NativeFileViewer.new())
    |> assign(:pending_requests, HandbeamProbe.PendingRequests.new())
