defmodule HandbeamProbe.ShareCopyTest do
  use ExUnit.Case, async: false

  alias HandbeamProbe.{ShareCopy, ShareIntake}
  ...
  test "owner death after success rolls back original dest not current draft", %{
    ...
    copy = fn _rec, ws ->
      ...
      receive do
        :proceed ->
          File.mkdir_p!(Path.join([ws["path"], ".handbeam", "shared", id]))
          File.write!(Path.join([ws["path"], ".handbeam", "shared", id, "notes.md"]), "copied")
          {:ok, [".handbeam/shared/#{id}/notes.md"]}
      end
    end
  ...
  test "rollback stays unconfirmable until dest is gone so a new copy cannot share the dir", %{
    ...
    copy = fn _rec, ws ->
      File.mkdir_p!(Path.join([ws["path"], ".handbeam", "shared", id]))
      File.write!(Path.join([ws["path"], ".handbeam", "shared", id, "notes.md"]), "first")
      {:ok, [".handbeam/shared/#{id}/notes.md"]}
    end
    ...
    rollback = fn ws, intake_id ->
      ...
      receive do
        :go -> HandbeamProbe.ShareWorkspaceImport.rollback(ws, intake_id)
      end
    end
