defmodule HandbeamProbe.ScriptEnvironmentTest do
  use ExUnit.Case, async: false

  alias Handbeam.Tool.{Registry, ScriptEnvironment}
  alias Handbeam.Tool.Builtin.RunElixirScript
  ...
  test "missing APIs are omitted, Mix.install is unavailable when Mix is absent, host claims are gated" do
    info = ScriptEnvironment.snapshot()
    ...
    test "#{mode} system prompt sends complete script guidance in #{protocol} provider JSON" do
      ...
      provider =
        if unquote(protocol) == :openai,
          do: Handbeam.Agent.Provider.StepFun,
          else: Handbeam.Agent.Provider.Anthropic
      ...
      assert {:ok, %{status: :completed}} = Handbeam.Agent.run("Check environment", opts)
