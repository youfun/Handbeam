defmodule HandbeamProbe.ScriptCSVTest do
  ...
  test "runtime dependency parses quoted UTF-8 CSV and roundtrips every field" do
    assert :nimble_csv in Application.spec(:handbeam_probe, :applications)
  ...
  test "the formal script tool can stream multiline CSV and write a roundtrip" do
    ...
    assert {:ok, _, _} =
             Handbeam.Tool.Builtin.RunElixirScript.execute(%{"path" => "verify.exs"}, %{
