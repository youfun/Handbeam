defmodule HandbeamProbe.NativeTimelineTest do
  use ExUnit.Case, async: true
  use Gettext, backend: HandbeamProbe.Gettext

  alias HandbeamProbe.NativeTimeline
  ...
  test "text-only attachments stay names and history restore uses upload refs" do
    ...
    assert length(images) == 1
    assert hd(images).props.src =~ "/.handbeam/uploads/#{conv_id}/"
  ...
  test "missing or naked paths fall back and never use a foreign workspace" do
    ...
    missing = %{
      ...
      "mime_type" => "image/png",
      "filename" => "gone.png",
      "relative_path" => Path.join([".handbeam", "uploads", conv_id, "gone.png"])
    }
  ...
  defp render(chat, toggles),
    do: NativeTimeline.render(HandbeamProbe.NativeLocalImage.resolve_sent(chat), toggles)

  defp render_sent_card(chat, open),
    do: NativeTimeline.render_sent_card(HandbeamProbe.NativeLocalImage.resolve_sent(chat), open)
