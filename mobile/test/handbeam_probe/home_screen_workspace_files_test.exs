defmodule HandbeamProbe.HomeScreenWorkspaceFilesTest do
  use Mob.ScreenCase, async: false
  use Gettext, backend: HandbeamProbe.Gettext

  alias HandbeamProbe.{HomeScreen, NativeWorkspaceOpen}
  ...
  test "chat open, close keeps draft, PDF uses snake_case snapshot, tree shares viewer", %{
    ...
    {:ok, conv} = Handbeam.ConversationStore.create(workspace["id"], title: "Files")
    ...
    assert text(view) =~ ".handbeam" or text(view) =~ ".dot"
    ...
    {:ok, _conv_b} = Handbeam.ConversationStore.create(workspace_b["id"], title: "B")
  ...
  test "opening the viewer does not stop a running chat", %{view: view} do
    ...
    assert {:ok, %{running?: true}} =
             Handbeam.Agent.Coordinator.status(assigns(view).chat.conversation["id"])
