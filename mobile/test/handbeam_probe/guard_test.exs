defmodule HandbeamProbe.GuardTest do
  @moduledoc """
  Static guards against merge regressions in `handbeam_probe/lib` (tech-debt plan WP1).
  ...
  @hex_palette "handbeam_probe/native_ui.ex"
  ...
  # WP8: every host / sigil payload is decoded once (`HandbeamProbe.Bridge.Inbound`,
  # `HandbeamProbe.Bridge.Payload`, `Handbeam.TranscriptEntry`), so no reader may fall
  ...
  describe "JNI string safety" do
    ...
    # Modified UTF-8 corrupts non-BMP text (emoji). WP5 moved handbeam_browser.c and
    # handbeam_notify.c to jbyteArray; keep empty so any new use fails.
    @jni_utf8_whitelist []
