defmodule HandbeamProbe.NativeFoundationTest do
  use ExUnit.Case, async: false
  use Gettext, backend: HandbeamProbe.Gettext

  alias HandbeamProbe.{NativeApproval, NativeChat, Platform}
  ...
  test "platform host uses the fake and never loads a NIF" do
    req =
      HandbeamProbe.Platform.Request.new(
        "platform_export",
        "req-1",
        1,
        self(),
    ...
    Application.put_env(:handbeam_probe, :platform_fake, fn cmd, _ -> {:ok, %{op: cmd.op}} end)
  ...
  test "cancel allocates a fresh command id and export_file authorizes first", %{
    ...
    Application.put_env(:handbeam_probe, :platform_fake, fn cmd, _ ->
  ...
  test "NIF wire map uses the C atom schema and engine_result decode" do
    req =
      HandbeamProbe.Platform.Request.new("platform_import", "rid-c", 4, self(), %{"path" => "x"})

    wired = HandbeamProbe.Platform.Nif.wire_map(req)
    ...
    assert {:ok, %HandbeamProbe.Bridge.Inbound.EngineResult{} = decoded} =
             HandbeamProbe.Platform.Nif.decode_engine_result(%{
               request_id: "rid-c",
               generation: 4,
  ...
  test "native send with a text attachment writes refs and no base64", %{workspace: workspace} do
    ...
    Application.put_env(:handbeam_probe, :staging_roots, [workspace["path"]])
    ...
    {:ok, entry} =
      Handbeam.Agent.TranscriptPersistence.append_inbound(conv["id"], "see",
    ...
    assert Handbeam.Attachments.inbound_ack(conv["id"], "corr-1") == :acknowledged
  end
