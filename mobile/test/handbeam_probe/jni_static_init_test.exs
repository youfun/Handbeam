defmodule HandbeamProbe.JniStaticInitTest do
  ...
  test "BrowserEngine nativeInitClass is a @JvmStatic external" do
    source =
      File.read!(
        Path.expand(
          "../../android/app/src/main/java/com/example/handbeam_probe/BrowserEngine.kt",
          __DIR__
        )
      )
  ...
  test "nativeShareIntakeReady returns before any enif call unless nif_ready" do
    source =
      File.read!(Path.expand("../../c_src/handbeam_browser.c", __DIR__))

    assert source =~ ~r/static atomic_bool nif_ready\s*=\s*0/

    {start, _} =
      :binary.match(source, "Java_com_example_handbeam_1probe_BrowserEngine_nativeShareIntakeReady")
  ...
  test "device App loads browser NIF before HomeScreen mount" do
    app = File.read!(Path.expand("../../lib/handbeam_probe/app.ex", __DIR__))
    engine = File.read!(Path.expand("../../lib/handbeam_probe/browser/engine.ex", __DIR__))
    assert engine =~ "HandbeamProbe.Browser.Nif.ensure_loaded()"
    install = :binary.match(app, "HandbeamProbe.Browser.Engine.install!()")
    mount = :binary.match(app, "Mob.Screen.start_root(HandbeamProbe.HomeScreen)")
    assert install
    assert mount
  ...
  test "MobBridge nativeInitPlatformClass is a @JvmStatic external" do
    source =
      File.read!(
        Path.expand(
          "../../android/app/src/main/java/com/example/handbeam_probe/MobBridge.kt",
          __DIR__
        )
      )
