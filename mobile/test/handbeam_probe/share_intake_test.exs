defmodule HandbeamProbe.ShareIntakeTest do
  use ExUnit.Case, async: false

  alias HandbeamProbe.ShareIntake

  setup do
    root = Path.join(System.tmp_dir!(), "share_intake_#{System.unique_integer([:positive])}")
    File.mkdir_p!(root)
    previous = Application.get_env(:handbeam_probe, :share_intake_root)
    ...
    on_exit(fn ->
      if previous,
        do: Application.put_env(:handbeam_probe, :share_intake_root, previous),
        else: Application.delete_env(:handbeam_probe, :share_intake_root)
  ...
  test "confirm is a read-only reviewability guard; ShareConfirm.begin owns the transition" do
    ...
    assert {:ok, :structured, _} = HandbeamProbe.ShareConfirm.begin(id)
    assert {:ok, %{"state" => "merged_current_process"}} = ShareIntake.get(id)
    assert {:error, :not_reviewable} = ShareIntake.confirm(id)
    assert {:error, :not_reviewable} = HandbeamProbe.ShareConfirm.begin(id)
  end
  ...
  test "write returns mkdir and rename failures as tuples" do
    ...
    previous = Application.get_env(:handbeam_probe, :share_intake_root)
    Application.put_env(:handbeam_probe, :share_intake_root, blocker)
    id = Ecto.UUID.generate()
    assert {:error, _reason} = ShareIntake.write(id, %{"state" => "pending_review"})
    Application.put_env(:handbeam_probe, :share_intake_root, previous)
    File.rm!(blocker)
  end
  ...
  test "explicit inbound ack cleans intake" do
    ...
    {:ok, _} =
      Handbeam.Agent.TranscriptPersistence.append_inbound(conv["id"], "shared", inbound_id: "ack-1")
  ...
  test "confirming recovery rolls back owned dest and returns to review" do
    {:ok, workspace} = Handbeam.WorkspaceStore.ensure_default!()
    id = Ecto.UUID.generate()
    dest = Path.join([workspace["path"], ".handbeam", "shared", id])
    File.mkdir_p!(dest)
    ...
    assert :ok = HandbeamProbe.ShareCopy.reconcile(notify: self(), abandon_all: true)
  ...
  test "receipt write failure does not delete manifest and does not resurrect" do
    id = Ecto.UUID.generate()
    write_fixture!(id, %{"state" => "pending_review", "text" => "x"})
    Application.put_env(:handbeam_probe, :share_receipt_write, fn _id, _state -> {:error, :eio} end)

    on_exit(fn -> Application.delete_env(:handbeam_probe, :share_receipt_write) end)
  ...
  test "root uses Host.data_dir when share_intake_root is unset" do
    Application.delete_env(:handbeam_probe, :share_intake_root)
    assert ShareIntake.root() == Path.join(Handbeam.Host.data_dir(), "share_intake")
  end
