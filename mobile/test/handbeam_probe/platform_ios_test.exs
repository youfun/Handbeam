defmodule HandbeamProbe.Platform.IOSTest do
  use ExUnit.Case, async: false

  alias HandbeamProbe.Bridge.Inbound
  alias HandbeamProbe.NativePlatform
  alias HandbeamProbe.Platform.{IOS, Nif, Request}
  alias HandbeamProbe.Platform.IOS.{Adapter, Import, Registry}
  ...
  setup do
    ...
    previous_roots = Application.get_env(:handbeam_probe, :staging_roots)
    previous_snap = Application.get_env(:handbeam_probe, :ios_snapshot_root)

    NativePlatform.put!(:ios)
    Process.put(:ios_test, self())
    Process.put(:ios_adapter_reply, :ok)
    Application.put_env(:handbeam_probe, :ios_platform_adapter, RecordingAdapter)
    ...
    Application.put_env(:handbeam_probe, :ios_snapshot_root, snap_root)
  ...
  test "adapter exception is not swallowed into a presented outcome" do
    Application.put_env(:handbeam_probe, :ios_platform_adapter, BoomAdapter)
  ...
  defp restore(key, nil), do: Application.delete_env(:handbeam_probe, key)
  defp restore(key, value), do: Application.put_env(:handbeam_probe, key, value)
