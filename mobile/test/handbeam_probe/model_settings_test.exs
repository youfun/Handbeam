defmodule HandbeamProbe.ModelSettingsTest do
  ...
  setup do
    ...
    vars = %{
      "HANDBEAM_WORKSPACE" => Path.join(dir, "workspace_a"),
    ...
    {:ok, ws_a} = Handbeam.WorkspaceStore.ensure_default!()
    path_b = Path.join(dir, "workspace_b")
    File.mkdir_p!(path_b)
    {:ok, ws_b} = Handbeam.WorkspaceStore.add(path_b, name: "B")
  ...
  test "delete without refs writes; referenced delete needs replacement; cancel does not write",
    ...
    assert Handbeam.Settings.global_model_ai().default_model == "beta/two"
    assert Handbeam.Settings.effective_model_ai(ws_b["path"]).default_model == "beta/two"
    assert Handbeam.Settings.effective_model_ai(ws_a["path"]).default_model == "beta/two"
  end
  ...
  test "workspace inherit removes overrides and does not copy effective values", %{
    ...
    assert Handbeam.Settings.global_model_ai().default_model == nil

    state = ModelSettings.action({:default_model, "alpha/one"}, state, ws_a)
    state = ModelSettings.action({:reasoning, "high"}, state, ws_a)
    assert Handbeam.Settings.global_model_ai().default_model == "alpha/one"
    ...
    {:ok, raw_a} = Handbeam.Settings.load_workspace_model_ai(ws_a["path"])
    ...
    assert Handbeam.Settings.effective_model_ai(ws_a["path"]).default_model == "alpha/one"

    state = ModelSettings.action(:inherit_all, state, ws_a)
    {:ok, raw_a} = Handbeam.Settings.load_workspace_model_ai(ws_a["path"])
    assert raw_a == %{}
    assert Handbeam.Settings.effective_model_ai(ws_a["path"]).reasoning == "high"

    state = ModelSettings.action({:reasoning, "minimal"}, ModelSettings.load(ws_b), ws_b)
    assert Handbeam.Settings.effective_model_ai(ws_b["path"]).reasoning == "minimal"
    assert Handbeam.Settings.effective_model_ai(ws_a["path"]).reasoning == "high"
  end
  ...
  test "memory switch and details enter runtime opts without saving follow labels", %{ws_a: ws} do
    ...
    assert Handbeam.Settings.global_model_ai().om_enabled
    ...
    settings = Handbeam.Settings.effective_model_ai(ws["path"])
  ...
  test "allowlist unconfigured, empty, and restricted behave as backend documents", %{
    ...
    settings_path = Handbeam.WorkspaceSettings.path(ws_a["path"])
    {:ok, existing} = Handbeam.WorkspaceSettings.load(ws_a["path"])
  ...
  test "provider delete lists model count and does not wipe other workspace settings", %{
    ...
    tools = %{"allow" => ["read"]}
    path = Handbeam.WorkspaceSettings.path(ws_b["path"])
    ...
    {:ok, settings} = Handbeam.WorkspaceSettings.load(ws_b["path"])
  ...
  test "locale strings cover the new provider catalog copy", %{ws_a: ws} do
    previous = Gettext.get_locale(HandbeamProbe.Gettext)
    on_exit(fn -> Gettext.put_locale(HandbeamProbe.Gettext, previous) end)

    # zh_CN must carry a real translation (not the msgid) for the catalog copy.
    Gettext.put_locale(HandbeamProbe.Gettext, "zh_CN")
  ...
  test "a broken workspace settings file does not crash load and is reported on delete", %{
    ...
    save_named(ws_a, "alpha", "one")
    save_named(ws_a, "alpha", "spare")
    File.write!(Handbeam.WorkspaceSettings.path(ws_b["path"]), "{ not jsonc")
    ...
    log =
      ExUnit.CaptureLog.capture_log(fn ->
        ...
        assert Handbeam.Settings.ModelRefs.blocking(state.confirm.refs) == []
