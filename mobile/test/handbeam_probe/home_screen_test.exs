defmodule HandbeamProbe.HomeScreenTest do
  ...
  test "workspaces state is loaded at mount and never nil across navigation", %{
    ...
    {:ok, conv} = Handbeam.ConversationStore.create(default_id, title: "Workspaces fixture")
    view = info(view, {:tap, {:conversation, conv["id"]}})
    assert %{mode: :list} = assigns(view).workspaces

    path_b = Path.join(dir, "workspace_ws_state")
    File.mkdir_p!(path_b)
    {:ok, workspace_b} = Handbeam.WorkspaceStore.add(path_b, name: "WS state")
  ...
  test "history overlays the unchanged chat and navigation identifies only user messages", %{
    ...
    {:ok, c} = Handbeam.ConversationStore.create("default", title: "Drawer fixture")
  ...
  test "blank send does not create a conversation; failed send preserves draft and conversation",
    ...
    assert length(Handbeam.ConversationStore.list_for_workspace("default")) == 1
  ...
  test "history and notification restore durable transcript without a LiveView", %{view: view} do
    {:ok, conversation} = Handbeam.ConversationStore.create("default", title: "Native history")
  ...
  test "native send reaches Coordinator and streams from a local fixture; stop cancels the run",
    ...
    id = assigns(view).chat.conversation["id"]
    assert {:ok, %{running?: true, queue_pid: queue}} = Handbeam.Agent.Coordinator.status(id)

    assert [%{message: %{content: "运行中追加中文🙂"}, metadata: %{deliver_as: :steer}}] =
             Handbeam.Agent.CandidateQueue.get_messages(queue)
  ...
  test "follow_up enqueue does not show the steer insertion hint", %{view: view} do
    ...
    refute notice_text(view)
    id = assigns(view).chat.conversation["id"]
    assert {:ok, %{queue_pid: queue}} = Handbeam.Agent.Coordinator.status(id)
  ...
  test "resend_pending sends only the undelivered item and keeps composer draft", %{view: view} do
    ...
    {:ok, _session} = Handbeam.PubSub.Session.start_or_get(session_id: id, model: "native-model")
    {:ok, queue} = Handbeam.Agent.CandidateQueue.start_link(session_id: id, owner: self())
    :ok = Handbeam.PubSub.Session.attach_run(id, self(), queue)

    chat =
      conversation
      |> NativeChat.load()
      |> NativeChat.note_enqueued(old_id, :steer, %{content: "retry me"})
      |> then(&%{&1 | pending: Handbeam.Agent.PendingMessages.mark_undelivered(&1.pending)})
    ...
    view = info(view, {:tap, {:resend_pending, old_id}})
    assert assigns(view).draft == "keep draft"
    assert length(Handbeam.Agent.CandidateQueue.get_messages(queue)) == 1
  end
  ...
  test "undo of queued item restores persisted attachments into the composer", %{view: view} do
    ...
    {:ok, queue} = Handbeam.Agent.CandidateQueue.start_link(session_id: id, owner: self())
    :ok = Handbeam.PubSub.Session.attach_run(id, self(), queue)
    :ok = Handbeam.Agent.CandidateQueue.enqueue(queue, "queued pic", message_id: mid)
  ...
  test "models save, remount, edit without exposing or erasing key, and select defaults", %{
    ...
    state = ModelSettings.action(:save_model, state, workspace)
    config = File.read!(Handbeam.Agent.ModelConfig.config_file_path()) |> Jason.decode!()
    ...
    assert Handbeam.Settings.effective_model_ai(workspace["path"]).default_model ==
             "test/native-model"
  ...
  test "model validation rejects HTTP and corrupt config without losing form", %{view: view} do
    ...
    refute File.exists?(Handbeam.Agent.ModelConfig.config_file_path())
    state = new_model(workspace)
    File.write!(Handbeam.Agent.ModelConfig.config_file_path(), "invalid json")
  ...
  test "assistant history uses markdown while user input stays literal", %{view: view} do
    ...
    for {role, body} <- [{"user", user}, {"assistant", reply}] do
      {:ok, _} =
        Handbeam.ConversationTranscriptStore.append(c["id"], %{"role" => role, "content" => body})
    end
  ...
  for {action, scope} <- [
    ...
    test "native approval #{action}/#{scope} resumes real Runner with correct persistence", %{
      ...
      {:ok, %{run_pid: runner}} =
        Handbeam.Agent.Coordinator.add_message(c["id"], "approval fixture",
      ...
      on_exit(fn -> Handbeam.Agent.Coordinator.cancel(c["id"]) end)
      release_turn(0)
  ...
  test "always-allow resume succeeds even when workspace rules cannot be saved", %{view: view} do
    ...
    path = workspace["path"]
    File.mkdir_p!(Path.join(path, ".handbeam"))
    settings_path = Handbeam.WorkspaceSettings.path(path)
  ...
  test "model, reasoning and permission controls share one composer row", %{view: view} do
    ...
    assert controls
    [model, gap, reasoning, spacer, permission] = controls.children
    ...
    assert length(reasoning.children) == length(Handbeam.Agent.Reasoning.levels())
    assert length(permission.children) == 3
    assert gap.type == :spacer
    assert gap.props.size == 6
    assert spacer.type == :box
    assert spacer.props.weight == 1
    assert controls.props.padding_bottom == 4
  end
  ...
  test "workspace switch isolates drafts, models, running events, and Coordinator path", %{
    ...
    {:ok, workspace_b} = Handbeam.WorkspaceStore.add(path_b, name: "Project B")

    {:ok, conv_a} = Handbeam.ConversationStore.create(workspace_a["id"], title: "Chat A")
    {:ok, conv_b} = Handbeam.ConversationStore.create(workspace_b["id"], title: "Chat B")

    {:ok, _} =
      Handbeam.ConversationTranscriptStore.append(conv_a["id"], %{
        "id" => "a1",
        "role" => "assistant",
    ...
    File.write!(
      Handbeam.WorkspaceSettings.path(workspace_b["path"]),
    ...
    assert_receive {:provider_request, request, _body}, 5_000
    id = assigns(view).chat.conversation["id"]
    assert {:ok, %{run_pid: runner, running?: true}} = Handbeam.Agent.Coordinator.status(id)
  ...
  test "creating an empty workspace does not inherit the previous chat or draft", %{view: view} do
    {:ok, conversation} = Handbeam.ConversationStore.create("default", title: "Keep me")
  ...
  test "notification switch reuses workspace apply and keeps the other draft", %{view: view} do
    workspace_a = assigns(view).workspace
    {:ok, extra} = HandbeamProbe.NativeWorkspaces.create_empty("Notify B")
    {:ok, conv_b} = Handbeam.ConversationStore.create(extra["id"], title: "Notify chat")
  ...
  test "late import receipts after cancel do not open a workspace", %{view: view, dir: dir} do
    ...
    dest = Path.join(HandbeamProbe.NativeWorkspaces.imported_root(), request_id)
    File.mkdir_p!(dest)
    ...
    refute Enum.any?(Handbeam.WorkspaceStore.list(), &(&1["name"] == "Late"))
  ...
  test "platform begin starts a typed Request and the C wire map has required keys", %{view: view} do
    ...
    install_platform_fake(fn req, _opts ->
      send(parent, {:platform_started, req, HandbeamProbe.Platform.Nif.wire_map(req)})
      {:ok, :async}
    end)
    ...
    assert %HandbeamProbe.Platform.Request{} = req
    ...
    assert {:error, :unknown_op} =
             HandbeamProbe.Platform.Request.for_kind(
               :camera,
               %{
  ...
  test "fake import joins the current draft and stale results are ignored", %{view: view} do
    ...
    imported = %Handbeam.Attachments.Imported{
  ...
  test "restored sent images open from message and attachment ids without sending", %{
    ...
    workspace = assigns(view).workspace
    {:ok, conv} = Handbeam.ConversationStore.create(workspace["id"])

    dest =
      Path.join(
        Handbeam.Uploads.ensure_conversation_dir!(workspace["path"], conv["id"]),
        "sent-1.png"
      )
    ...
    relative = Path.join([".handbeam", "uploads", conv["id"], "sent-1.png"])

    {:ok, entry} =
      Handbeam.Agent.TranscriptPersistence.append_inbound(conv["id"], "历史图片",
        attachments: [
          %{
            "id" => "sent-1",
            "kind" => "image",
  ...
  test "switching drafts cancels pending ops and only deletes importer-owned files", %{
    ...
    Application.put_env(:handbeam_probe, :staging_roots, [owned])
    on_exit(fn -> Application.delete_env(:handbeam_probe, :staging_roots) end)
  ...
  test "opening another conversation cancels pending attachment work", %{view: view} do
    ...
    {:ok, conversation} = Handbeam.ConversationStore.create(assigns(view).workspace["id"])
  ...
  test "image result after model switch is kept, warns, and cannot be sent", %{
    ...
    socket = HandbeamProbe.HomeScreen.Platform.add_attachment(view.socket, att)
    ...
    assert assigns(view).chat == nil
    assert length(assigns(view).pending_attachments) == 1
    assert Handbeam.ConversationStore.list() == []
  ...
  test "unknown image capability warns without discarding externally imported images", %{
    ...
    File.write!(path, <<0>>)
    :ok = Handbeam.Agent.ModelConfig.add_model("stepfun", "custom", %{"name" => "Custom"})
  ...
  test "photo picker begin uses one request id and stale batch files are released", %{
    ...
    File.write!(path, "img")
    Application.put_env(:handbeam_probe, :staging_roots, [dir])
    on_exit(fn -> Application.delete_env(:handbeam_probe, :staging_roots) end)
  ...
  test "iOS photo pick waits for files_picked, imports into composer, and ignores late results",
       %{view: view} do
    previous_platform = Application.get_env(:handbeam_probe, :native_platform)
    previous_adapter = Application.get_env(:handbeam_probe, :ios_platform_adapter)
    ...
    Application.put_env(
      ...
      HandbeamProbe.HomeScreenTest.IOSAdapter
    )

    Application.put_env(:handbeam_probe, :ios_test_pid, self())
    Application.put_env(:handbeam_probe, :ios_adapter_reply, :ok)

    staging = Path.join(System.tmp_dir!(), "hs_ios_#{System.unique_integer([:positive])}")
    File.mkdir_p!(Path.join(staging, "draft"))
    Application.put_env(:handbeam_probe, :staging_roots, [staging])

    on_exit(fn ->
      File.rm_rf!(staging)
      Application.delete_env(:handbeam_probe, :staging_roots)
      Application.delete_env(:handbeam_probe, :ios_test_pid)
      Application.delete_env(:handbeam_probe, :ios_adapter_reply)
      ...
      if previous_adapter,
        do: Application.put_env(:handbeam_probe, :ios_platform_adapter, previous_adapter),
        else: Application.delete_env(:handbeam_probe, :ios_platform_adapter)
    end)
  ...
  test "share confirm merges draft without sending; ack is explicit", %{view: view} do
    ...
    assert MapSet.member?(assigns(view).merged_intake_ids, id)
    assert assigns(view).chat == nil
    assert {:ok, %{"state" => "merged_current_process"}} = HandbeamProbe.ShareIntake.get(id)
  ...
  test "failed merge persistence leaves draft untouched", %{view: view} do
    ...
    assert {:ok, %{"state" => "pending_review"}} = HandbeamProbe.ShareIntake.get(id)
  end
  ...
  test "importing intake cannot confirm into draft", %{view: view} do
    ...
    refute MapSet.member?(assigns(view).merged_intake_ids, id)
    assert {:ok, %{"state" => "importing"}} = HandbeamProbe.ShareIntake.get(id)
  end
  ...
  test "overflow share notify is visible and does not add review", %{view: view} do
    ...
    assert notice_text(view) =~
             gettext("Pending shares are full (max %{max}). Confirm or discard one first.",
               max: HandbeamProbe.ShareIntake.max_pending()
             )
  ...
  test "workspace copy confirm is queued and late discard rolls back", %{view: view} do
    ...
    Application.put_env(:handbeam_probe, :share_io_notify, self())
    on_exit(fn -> Application.delete_env(:handbeam_probe, :share_io_notify) end)
    ...
    dest = Path.join([workspace["path"], ".handbeam", "shared", a])
    File.mkdir_p!(dest)
    ...
    view =
      info(
        ...
        {:share_workspace_result,
         %{
           intake_id: a,
           target: HandbeamProbe.ShareConfirm.target_from(workspace, nil, a),
           result: {:ok, [".handbeam/shared/#{a}/notes.md"]}
         }}
      )
  ...
  test "late workspace copy after workspace switch does not merge current draft", %{view: view} do
    ...
    dest = Path.join([original["path"], ".handbeam", "shared", a])
    File.mkdir_p!(dest)
    File.write!(Path.join(dest, "notes.md"), "shared notes")
    ...
    refute MapSet.member?(assigns(view).merged_intake_ids, a)
    refute File.exists?(Path.join([other["path"], ".handbeam", "shared", a]))
  end
  ...
  test "lost share send ack is visible and does not auto retry", %{view: view} do
    ...
    refute assigns(view).last_send_ack
    assert {:ok, %{"state" => "outcome_unknown"}} = HandbeamProbe.ShareIntake.get(id)
  end
  ...
  test "a delivery request past its deadline is cancelled and reported once", %{view: view} do
    ...
    Application.put_env(:handbeam_probe, :android_intent_await_ms, 0)
    on_exit(fn -> Application.delete_env(:handbeam_probe, :android_intent_await_ms) end)
  ...
  test "host directory picker opens the in-app folder browser and lists off-screen", %{
    ...
    Application.put_env(:handbeam_probe, :directory_picker_screen, self())
  ...
  test "directory picker is unavailable without a screen" do
    Application.put_env(:handbeam_probe, :directory_picker_screen, :no_such_screen_process)
    on_exit(fn -> Application.delete_env(:handbeam_probe, :directory_picker_screen) end)
    assert HandbeamProbe.DirectoryPicker.request_directory_picker(%{}) == {:error, :unavailable}
  end

  describe "mount locale" do
    setup do
      previous_env = Application.get_env(:handbeam_probe, :locale)
      previous_locale = Gettext.get_locale()

      on_exit(fn ->
        if previous_env,
          do: Application.put_env(:handbeam_probe, :locale, previous_env),
          else: Application.delete_env(:handbeam_probe, :locale)
    ...
    test "defaults to zh_CN when nothing is configured" do
      Application.delete_env(:handbeam_probe, :locale)
      view = settle(mount_screen(HomeScreen))
      assert Gettext.get_locale(HandbeamProbe.Gettext) == HandbeamProbe.Gettext.default_locale()
    ...
    test "renders English when :locale is configured as en" do
      Application.put_env(:handbeam_probe, :locale, "en")
      view = settle(mount_screen(HomeScreen))
      assert Gettext.get_locale(HandbeamProbe.Gettext) == "en"
    ...
    test "unknown locale values fall back to the default" do
      Application.put_env(:handbeam_probe, :locale, "fr")
      assert HandbeamProbe.Gettext.locale() == HandbeamProbe.Gettext.default_locale()
      Application.put_env(:handbeam_probe, :locale, :en)
      assert HandbeamProbe.Gettext.locale() == "en"
      Application.put_env(:handbeam_probe, :locale, "zh-CN")
      assert HandbeamProbe.Gettext.locale() == "zh_CN"
    end
  end
  ...
  defmodule IOSAdapter do
    @behaviour HandbeamProbe.Platform.IOS.Adapter
    ...
    defp pid, do: Application.get_env(:handbeam_probe, :ios_test_pid, self())
    defp reply, do: Application.get_env(:handbeam_probe, :ios_adapter_reply, :ok)
  end

  defp install_platform_fake(fun) do
    Application.put_env(:handbeam_probe, :platform_fake, fun)
    on_exit(fn -> Application.delete_env(:handbeam_probe, :platform_fake) end)
  end
  ...
  defp share_root!(prefix) do
    ...
    File.mkdir_p!(root)
    Application.put_env(:handbeam_probe, :share_intake_root, root)
    on_exit(fn -> Application.delete_env(:handbeam_probe, :share_intake_root) end)
    root
  end

  defp write_share_fixture!(id, rec) do
    dir = Path.join(HandbeamProbe.ShareIntake.root(), id)
    File.mkdir_p!(dir)
  ...
  defp notice_text(view), do: HandbeamProbe.HomeScreen.Notice.text(assigns(view).notice)
