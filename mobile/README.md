...
iOS app icons are checked in under `ios/Assets.xcassets/AppIcon.appiconset`.
They reuse the Android vector mark and launcher colors, with opaque PNGs for
iPhone, iPad, and the 1024px App Store slot. The same command also refreshes
the rounded legacy Android mipmaps (`mipmap-*dpi/ic_launcher.png`). To
regenerate them on macOS:

```bash
swift script/generate_ios_icons.swift
```

`mix mob.release --ios` compiles this catalog with `actool` and merges its
generated `CFBundleIcons` / `CFBundlePrimaryIcon` metadata into the final
Info.plist. Keep the catalog in source control: a top-level `CFBundleIconName`
alone does not supply the required icon resources. `ios/release_device.sh` is
regenerated by Mob on every release, so do not put persistent fixes there.
After changing icons, rebuild the IPA; an already signed IPA is unchanged.
