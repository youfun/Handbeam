...
Java_com_example_handbeam_1probe_BrowserEngine_nativeInitClass(JNIEnv* env, jclass cls) {
...
JNIEXPORT void JNICALL
Java_com_example_handbeam_1probe_MobBridge_nativeInitPlatformClass(JNIEnv* env, jclass cls) {
    pthread_mutex_lock(&g_jni_cache_lock);
    if (!g_bridge_cls) {
        jclass global = (*env)->NewGlobalRef(env, cls);
...
static int onload(ErlNifEnv* env, void** priv, ERL_NIF_TERM info) {
    ...
    g_pending_lock = enif_mutex_create("handbeam_browser_pending");
...
ERL_NIF_INIT(handbeam_browser, nif_funcs, onload, NULL, NULL, NULL)

/* JNI callbacks from BrowserEngine */
JNIEXPORT void JNICALL
Java_com_example_handbeam_1probe_BrowserEngine_nativeDeliver(
...
JNIEXPORT void JNICALL
Java_com_example_handbeam_1probe_BrowserEngine_nativeHandback(JNIEnv* env, jclass cls, jstring sessionId, jlong pid_unused) {
    (void)env; (void)cls; (void)sessionId; (void)pid_unused;
    /* Handback is delivered through the OTP session API from Elixir, not JNI. */
}

JNIEXPORT void JNICALL
Java_com_example_handbeam_1probe_BrowserEngine_nativeShareIntakeReady(
