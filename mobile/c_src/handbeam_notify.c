/* handbeam_notify — Android run-lifecycle notifications.
 *
 * Compiled with -DSTATIC_ERLANG_NIF -DSTATIC_ERLANG_NIF_LIBNAME=handbeam_notify
 * so ERL_NIF_INIT emits handbeam_notify_nif_init().
 */
#include <erl_nif.h>
#include <jni.h>
#include <stdint.h>
...
Java_com_example_handbeam_1probe_AgentNotify_nativeInitClass(JNIEnv *env, jclass cls) {
  if (notify_class)
    return;
...
Java_com_example_handbeam_1probe_AgentNotify_nativeOpenConversation(JNIEnv *jni, jclass cls, jbyteArray json) {
  (void)cls;
...
ERL_NIF_INIT(handbeam_notify, nif_funcs, onload, NULL, NULL, NULL)
