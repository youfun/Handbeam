%% handbeam_probe.erl — BEAM bootstrap for HandbeamProbe.
%% Called by the iOS/Android native launcher via -eval 'handbeam_probe:start().'.
%% Starts the OTP ecosystem in order, then hands off to the Elixir app module.
-module(handbeam_probe).
-export([start/0]).
...
start() ->
    ...
    step(5, fun() -> 'Elixir.HandbeamProbe.App':start() end),
