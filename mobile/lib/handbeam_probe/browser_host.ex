defmodule HandbeamProbe.BrowserHost do
  @moduledoc """
  JS helpers and reply formatting for the on-device browser pane.

  The pane itself lives on `HandbeamProbe.HomeScreen` so chat stays visible.
  """
  ...
  def format_reply(msg) do
    case Handbeam.JSON.encode(msg) do
      {:ok, json} -> json
      _ -> inspect(msg)
    end
  end
  ...
  def snapshot_js do
    ...
    (function(){
      ...
      for (var i=0;i<nodes.length && items.length<40;i++){
        ...
        if(!t) continue;
        el.setAttribute('data-handbeam-ref', String(items.length));
        items.push({ref:String(items.length), tag:(el.tagName||'').toLowerCase(), text:t});
      }
      var body = (document.body && document.body.innerText) ? document.body.innerText : '';
      send({kind:'handbeam_browser', action:'snapshot', url:location.href, title:document.title||'', text:body.slice(0,4000), items:items});
    })();
    """
  end
  ...
  def wrap_eval(js) do
    ...
    (function(){
      ...
      try {
        ...
        send({kind:'handbeam_browser', action:'eval', ok:true, result:String(result)});
      } catch(e) {
        send({kind:'handbeam_browser', action:'eval', ok:false, error:String(e)});
      }
    })();
    """
  end
  ...
  def click_js(ref) do
    selector = js_string(~s([data-handbeam-ref="#{ref}"]))

    """
    (function(){
      function send(d){ try { window.mob.send(d); } catch(e) {} }
      var el = document.querySelector(#{selector});
      if(!el){ send({kind:'handbeam_browser', action:'click', ok:false, error:'not found'}); return; }
      el.click();
      send({kind:'handbeam_browser', action:'click', ok:true});
    })();
    """
  end
  ...
  def fill_js(ref, value) do
    ...
    (function(){
      function send(d){ try { window.mob.send(d); } catch(e) {} }
      var el = document.querySelector(#{selector});
      if(!el){ send({kind:'handbeam_browser', action:'fill', ok:false, error:'not found'}); return; }
      el.focus();
      el.value = #{value};
      el.dispatchEvent(new Event('input', {bubbles:true}));
      el.dispatchEvent(new Event('change', {bubbles:true}));
      send({kind:'handbeam_browser', action:'fill', ok:true});
    })();
    """
  end
