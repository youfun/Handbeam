defmodule HandbeamProbe.NativeApproval do
  @moduledoc """
  ...
  `"tool_call_id"`, …): `HandbeamProbe.NativeChat.event_payload/1` normalises the
  ...
  defp remember(requests, path, action, :always) do
    ...
    Enum.reduce_while(requests, :ok, fn req, :ok ->
      ...
      case Handbeam.WorkspaceSettings.append_tool_rule(path, list, pattern) do
        :ok -> {:cont, :ok}
