defmodule HandbeamProbe.ModelSettings do
  @moduledoc """
  Native model configuration form, backed by Handbeam's existing stores.

  Facade for `HandbeamProbe.HomeScreen`: owns the state shape, `load/2`, and the
  `action/3` dispatch. Logic lives in `HandbeamProbe.ModelSettings.Providers`
  ...
  `find_refs_async/4` run them under `HandbeamProbe.TaskSupervisor` for callers
  ...
  @doc """
  `load/2` under `HandbeamProbe.TaskSupervisor`.
  ...
  @doc """
  Reference scan for a delete under `HandbeamProbe.TaskSupervisor`.
