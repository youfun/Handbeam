defmodule HandbeamProbe.AndroidIntent do
  ...
  `HandbeamProbe.Platform.open_url_request/4`; `open_file` / `share_file` are the
  present step of the artifact sequence, `HandbeamProbe.Platform.present_request/6`,
  fed with the snapshot that `HandbeamProbe.NativeArtifactDelivery` pinned at
  approval time (`Handbeam.ExportSnapshot.Binding`). The UI tap path builds the
  same requests through the same functions.

  The wait is isolated in a spawned process so the caller's mailbox is never
  drained; the reply is decoded by `HandbeamProbe.Bridge.Inbound.engine_result/1`
