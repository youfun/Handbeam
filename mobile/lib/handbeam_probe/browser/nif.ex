defmodule HandbeamProbe.Browser.Nif do
  @moduledoc """
  ...
  def command(cmd, opts \\ []) do
    case Application.get_env(:handbeam_probe, :browser_engine_fake) do
      fun when is_function(fun, 2) -> fun.(cmd, opts)
      _ -> device_command(cmd)
    end
  end

  # Every key `c_src/handbeam_browser.c` reads with enif_get_map_value, plus the
  ...
  defp device_command(cmd) do
    case Code.ensure_loaded(:handbeam_browser) do
      {:module, :handbeam_browser} -> :handbeam_browser.command(wire_map(cmd))
