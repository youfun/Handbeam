defmodule HandbeamProbe.HomeScreen.Platform do
  @moduledoc """
  Composer attachments and the typed `HandbeamProbe.Platform` request cycle:
  ...
  `HandbeamProbe.Bridge.Inbound.EngineResult` when it reaches `handle/2`.
  Requests are correlated through `HandbeamProbe.HomeScreen.Requests`
  (`HandbeamProbe.PendingRequests`, scope `:composer`): the wire `generation`
  ...
  `HandbeamProbe.NativeArtifactDelivery.handle_result/3`.
  ...
  alias HandbeamProbe.Bridge.{Inbound, Payload}
  alias HandbeamProbe.HomeScreen.{Async, Notice, Requests}
  alias HandbeamProbe.{NativeArtifactDelivery, NativeComposer, PendingRequests}
  ...
  defp begin(socket, :pick_photos, payload) when is_map(payload) do
    socket = HandbeamProbe.NativeModelInputs.check(socket, ["image"])
  ...
  defp start_request(socket, kind, payload) do
    ...
    with {:ok, req} <- Request.for_kind(kind, ctx, self(), payload),
         {:ok, _} <- HandbeamProbe.Platform.start(req) do
  ...
  def handle_timeout(socket, %PendingRequests.Entry{ref: request_id, ctx: ctx} = entry)
    ...
    _ = HandbeamProbe.Platform.cancel(self(), request_id, entry.generation)
  ...
  def add_attachment(socket, attachment) do
    ...
    case Handbeam.Attachments.validate_batch(current ++ [att]) do
      :ok ->
        socket
    ...
    if paths != [] do
      _ =
        Async.fire(:draft_files_removed, fn ->
          Enum.each(paths, &HandbeamProbe.Platform.safe_rm_controlled/1)
        end)
    end
