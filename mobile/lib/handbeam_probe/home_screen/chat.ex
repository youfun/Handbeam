defmodule HandbeamProbe.HomeScreen.Chat do
  @moduledoc """
  ...
  `Handbeam.PubSub.Session` events projected by `HandbeamProbe.NativeChat`.
  ...
  alias HandbeamProbe.HomeScreen.{Nav, Notice, Platform, Share}
  ...
  def handle({:tap, {:tool_action, conversation_id, kind, id}}, socket)
    ...
    if current?(socket, conversation_id) do
      ...
      Task.Supervisor.start_child(HandbeamProbe.TaskSupervisor, fn ->
  ...
  def handle({:alert, :cancel_all_runs}, socket) do
    Handbeam.Runtime.cancel_all_runs()
    socket
  end
  ...
  defp start_send(socket, composer_mode) do
    a = socket.assigns
    content = HandbeamProbe.WritingPhotoReviews.compose_user_text(a)
    ...
    cond do
      ...
      HandbeamProbe.NativeModelInputs.warning(a) != nil ->
        HandbeamProbe.NativeModelInputs.check(socket)
  ...
  def send_after_intake_pending(socket, inbound_id, deliver_as \\ :steer, composer_mode \\ :chat) do
    ...
    case NativeChat.send_message(a.workspace, a.chat.conversation, content, a.pending_attachments,
      ...
      {:error, {:model_input, _, _}} ->
        ...
        |> assign(:models, HandbeamProbe.ModelSettings.load(a.workspace))
  ...
  defp restore_undelivered(chat, id) do
    %{chat | pending: Handbeam.Agent.PendingMessages.put_status(chat.pending, id, :undelivered)}
  end
