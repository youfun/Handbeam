defmodule HandbeamProbe.HomeScreen.Requests do
  @moduledoc """
  Socket-level facade over the `pending_requests` assign, a
  `HandbeamProbe.PendingRequests`. Every domain registers, takes and bumps
  ...
  def timeout_ms(kind) when kind in @picker_kinds,
    do: Application.get_env(:handbeam_probe, :platform_picker_timeout_ms)
  ...
  def timeout_ms(kind)
      ...
      do: Application.get_env(:handbeam_probe, :android_intent_await_ms, @default_await_ms)
