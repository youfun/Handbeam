defmodule HandbeamProbe.HomeScreen.FileNav do
  @moduledoc """
  ...
  `HandbeamProbe.TaskSupervisor` and returns as
  ...
  A workspace switch chosen here is applied by `HandbeamProbe.HomeScreen.Nav`.
  """

  import Mob.Socket, only: [assign: 3]

  alias HandbeamProbe.HomeScreen.{Async, Nav, Notice}
  ...
  def handle({:files, :cancelled}, socket) do
    if HandbeamProbe.NativePlatform.ios?() and
         HandbeamProbe.Platform.IOS.consume_files_event(:cancelled) == :handled do
      socket
    else
  ...
  def handle({:files, :picked, items}, socket) do
    if HandbeamProbe.NativePlatform.ios?() and
         HandbeamProbe.Platform.IOS.consume_files_event({:picked, items}) == :handled do
      socket
    else
  ...
  @doc """
  `Handbeam.Host` directory picker (see `HandbeamProbe.DirectoryPicker`): open the
