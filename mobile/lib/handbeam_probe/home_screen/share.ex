defmodule HandbeamProbe.HomeScreen.Share do
  @moduledoc """
  ...
  FIFO under `HandbeamProbe.TaskSupervisor` and the result comes back as
  `{:share_intakes_ready, generation, {reviews, unknown?}}` (scope `:share_intakes_ready`
  in `HandbeamProbe.PendingRequests`, latest listing wins); `begin_send/2`
  ...
  alias HandbeamProbe.HomeScreen.{Async, Nav, Notice, Platform}
  alias HandbeamProbe.{ShareConfirm, ShareCopy, ShareIntake, ShareWorkspaceImport}
  ...
  defp discard(socket, intake_id) do
    ctx = HandbeamProbe.NativeComposer.context(socket)

    case HandbeamProbe.Platform.Request.for_kind(:share_discard, ctx, self(), %{
           "intake_id" => intake_id
         }) do
      {:ok, req} -> _ = HandbeamProbe.Platform.start(req)
      {:error, _} -> :ok
    end
