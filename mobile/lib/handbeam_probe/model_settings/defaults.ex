defmodule HandbeamProbe.ModelSettings.Defaults do
  @moduledoc """
  Model/AI defaults (scope, default model, reasoning, inherit) for the native
  model settings. Persists through `Handbeam.Settings.ModelAIOverride`.
  """

  use Gettext, backend: HandbeamProbe.Gettext
  alias Handbeam.Agent.ModelConfig
  alias Handbeam.Settings.ModelAIOverride
  ...
  def form_for_scope(:global, _workspace), do: Handbeam.Settings.global_model_ai()

  def form_for_scope(:workspace, workspace) do
    case Handbeam.Settings.fetch_effective_model_ai(workspace["path"]) do
      {:ok, settings} -> settings
      {:error, _} -> Handbeam.Settings.global_model_ai()
    end
  end
