defmodule HandbeamProbe.ModelSettings.Providers do
  @moduledoc """
  ...
  replacement) and persists through `Handbeam.Agent.ModelConfig`. Reference
  scanning is `Handbeam.Settings.ModelRefs`. Every function takes and returns the
  `HandbeamProbe.ModelSettings` state map.
  """

  use Gettext, backend: HandbeamProbe.Gettext
  alias Handbeam.Agent.ModelConfig
  alias Handbeam.Settings.{ModelCatalog, ModelRefs}
  alias HandbeamProbe.ModelSettings
  alias HandbeamProbe.ModelSettings.Forms
  alias HandbeamProbe.SettingsSupport
  ...
  `refs_result` is `Handbeam.Settings.ModelRefs.find/3` output; skipped workspaces
