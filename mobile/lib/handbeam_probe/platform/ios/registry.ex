defmodule HandbeamProbe.Platform.IOS.Registry do
  @moduledoc """
  ...
  def snapshot_dir do
    Application.get_env(:handbeam_probe, :ios_snapshot_root) || default_snapshot_dir()
  end
    ...
    case System.get_env("MOB_CACHE_DIR") do
      ...
      _ ->
        case System.get_env("MOB_DATA_DIR") do
          ...
          _ ->
            Path.join(System.tmp_dir!(), "handbeam_ios_export_snapshots")
        end
    end
  end
  ...
  defp mime_for(name) do
    {:ok, _kind, mime} = Handbeam.WorkspaceFiles.kind_from_name(name)
    mime
  end
