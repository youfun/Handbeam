defmodule HandbeamProbe.ShareIntake do
  @moduledoc """
  ...
  alias HandbeamProbe.ShareCopy
  alias HandbeamProbe.ShareIntake.Lock
  ...
  def root do
    Application.get_env(:handbeam_probe, :share_intake_root) ||
      Path.join(Handbeam.Host.data_dir(), "share_intake")
  end
  ...
  `HandbeamProbe.ShareConfirm.begin/2` (`mark_confirming/2` for workspace copy,
  ...
  def schedule_cleanup(intake_id, opts \\ []) when is_binary(intake_id) do
    supervisor = Keyword.get(opts, :supervisor, HandbeamProbe.TaskSupervisor)
  ...
  def write_receipt(intake_id, state) when is_binary(intake_id) and is_binary(state) do
    case Application.get_env(:handbeam_probe, :share_receipt_write) do
  ...
  defp notify_pids(opts) do
    [Keyword.get(opts, :notify), Application.get_env(:handbeam_probe, :share_io_notify)]
