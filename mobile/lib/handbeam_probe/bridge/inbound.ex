defmodule HandbeamProbe.Bridge.Inbound do
  @moduledoc """
  ...
  Three shapes arrive from outside the BEAM:
    ...
    * `{:engine_result, map}` — `c_src/handbeam_browser.c` echoes a platform
  ...
  documents stay string-keyed maps because that is the `Handbeam.Attachments`
  ...
  defmodule EngineResult do
    @moduledoc "Decoded `{:engine_result, map}` envelope. `body` is `HandbeamProbe.Bridge.Inbound.body/0`."
    ...
    @type t :: %__MODULE__{
            ...
            url: String.t() | nil,
            body: HandbeamProbe.Bridge.Inbound.body()
          }
  end

  defmodule Notification do
    @moduledoc "Decoded `{:notification, map}`; `data` is flattened to the two ids Handbeam uses."
  ...
  defmodule Snapshot do
    ...
    `Handbeam.ExportSnapshot.Binding` stores it.
    """
    defstruct [
      :snapshot_id,
      :owner_request_id,
  ...
  JSON body, the atom-keyed meta `Handbeam.ExportSnapshot.Binding` stores, or an
  existing struct.
  """
