defmodule HandbeamProbe.WritingPhotoReviews do
  @moduledoc """
  ...
  @doc "Authoritative path under `Handbeam.Host.priv_dir/0`."
  def skill_path do
    Path.join(Handbeam.Host.priv_dir(), @skill_rel)
  end

  @doc """
  Read and strip frontmatter. Host priv first (packed APK). Mix tests may
  fall back to the `:handbeam_probe` app priv when Host still points at `:handbeam`.
  """
  ...
  defp mix_fallback_paths do
    probe = Application.app_dir(:handbeam_probe, Path.join("priv", @skill_rel))
