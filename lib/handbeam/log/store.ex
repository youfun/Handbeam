defmodule Handbeam.Log.Store do
  @moduledoc """
  ...
  ## Example
      ...
      {:ok, store} = Handbeam.Log.Store.start_link(name: :audit_store)
      {:ok, event} = Handbeam.Log.Event.new(kind: :session, level: :info, message: "start")
      Handbeam.Log.Store.append(store, event)
      Handbeam.Log.Store.list(store)
      # => [%Handbeam.Log.Event{...}]

  ## Future integration

  When structured logging is integrated into the agent pipeline:
    * Start a store per session and pass it through `Handbeam.Agent.State`
    * Wire `Handbeam.Log.Store.append/2` calls into `Handbeam.Agent.Middleware.Logger`
    * Use `Handbeam.Log.Store.filter/2` for per-session or per-kind audit views
  """

  use Agent

  alias Handbeam.Log.Event
