defmodule Handbeam.Log.Formatter do
  @moduledoc """
  Serialization and deserialization for `Handbeam.Log.Event`.

  Provides `to_map/1`, `to_json/1`, and `from_map/1` for
  converting events to/from external representations.

  > **Redaction note:** Always apply `Handbeam.Log.Redactor.redact/1` to
  ...
  this formatter can be wired into:
    ...
    * `Handbeam.Agent.Middleware.Logger` — log turn events as structured JSON
    * `Handbeam.PubSub.AgentEvent` — serialize for audit persistence
  ...
  Uses `Handbeam.JSON.encode!/1` — the event is expected to be pre-redacted
