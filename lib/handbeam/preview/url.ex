defmodule Handbeam.Preview.URL do
  @moduledoc """
  ...
  def return_conversation_id(%URI{scheme: "handbeam", host: "c", path: path})
  ...
  def return_conversation_id(%URI{scheme: "handbeam", host: host, path: path})
      when is_binary(host) and is_binary(path) do
    # handbeam://c/<id> sometimes parses host as "c" and path as "/id".
    :error
  end

  def return_conversation_id(%URI{scheme: "handbeam", path: path, host: nil})
      when is_binary(path) do
