defmodule Handbeam.Memory.ObservationStore do
  @moduledoc """
  JSONL-based observation persistence.

  Stores observations per conversation in `~/.handbeam/observations/<conversation_id>.jsonl`.
  ...
  alias Handbeam.Memory.Observation
  ...
  def base_dir, do: Handbeam.Home.expand("~/.handbeam/observations")
  ...
  def append(conversation_id, %Observation{} = obs) when is_binary(conversation_id) do
    ...
    with :ok <- ensure_dir(path),
         {:ok, json} <- Handbeam.JSON.encode(Observation.to_map(obs)),
          ...
          case Handbeam.JSON.decode(line) do
            {:ok, map} ->
              if MapSet.member?(id_set, map["id"]) do
                ...
                map |> Map.put("metadata", updated_meta) |> Handbeam.JSON.encode!()
              else
                line
              end
  ...
  defp read_reverse_lines(io, limit) do
    ...
    |> Enum.take(limit)
    |> Enum.map(fn line ->
      case Handbeam.JSON.decode(String.trim(line)) do
