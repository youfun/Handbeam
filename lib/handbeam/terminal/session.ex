defmodule Handbeam.Terminal.Session do
  @moduledoc """
  ...
  Registered in `Handbeam.Terminal.Registry` under `{workspace_id, name}`.
  """
  ...
  def handle_call({:snapshot, opts}, _from, state) do
    ...
    result =
      with {:ok, text} <- Ghostty.Terminal.snapshot(state.term, format),
        ...
        %{content: content} =
          Handbeam.Utils.Truncate.truncate(text, :tail, max_bytes: @max_snapshot_bytes)
  ...
  defp persist_output(state, data) do
    dir = Path.join([state.workspace_path, ".handbeam", "terminals"])
    File.mkdir_p!(dir)
