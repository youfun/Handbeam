defmodule Handbeam.Workspace do
  @moduledoc """
  ...
  1. `HANDBEAM_WORKSPACE` environment variable
  2. `Application.get_env(:handbeam, :workspace_root)`
  3. Fallback: `~/.handbeam/workspace`
  """

  alias Handbeam.Security.PathValidator

  @default_root "~/.handbeam/workspace"

  @doc """
  Return the expanded workspace root directory.

  Resolution order:
    1. `HANDBEAM_WORKSPACE` env var
    2. `Application.get_env(:handbeam, :workspace_root)`
    3. `~/.handbeam/workspace`
  """
  @spec root() :: String.t()
  def root do
    from_env = System.get_env("HANDBEAM_WORKSPACE")
    from_app = Application.get_env(:handbeam, :workspace_root)

    cond do
      is_binary(from_env) and from_env != "" -> Handbeam.Home.expand(from_env)
      is_binary(from_app) -> Handbeam.Home.expand(from_app)
      true -> Handbeam.Home.expand(@default_root)
    end
  end
  ...
  Delegates to `Handbeam.Security.PathValidator.validate_within_workspace/2`.
  """
