defmodule Handbeam.Uploads do
  @moduledoc """
  Workspace-local uploads storage (no S3/object storage).

  Stores uploads under:

      <workspace_path>/.handbeam/uploads/<conversation_id>/<uuid>.<ext>
  """

  alias Handbeam.Attachments
  alias Handbeam.Attachments.{Access, Imported}
  ...
  def promote_path(workspace_path, conversation_id, source_path, meta, opts \\ [])
    ...
    with true <- is_binary(ext),
         ...
         relative <- Path.join([".handbeam", "uploads", conversation_id, "#{id}.#{ext}"]),
