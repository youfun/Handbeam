defmodule Handbeam.Extension.Loader do
  @moduledoc """
  ...
  Discovery locations:
  - user: ~/.handbeam/extensions
  - project: {workspace}/.handbeam/extensions
  ...
  defmodule Result do
    ...
    @type t :: %__MODULE__{
            extensions: [Handbeam.Extension.t()],
  ...
  Scans `{home}/.handbeam/extensions/` for extension directories.
  """
  @spec from_user(String.t()) :: Result.t()
  def from_user(home) do
    dir = Path.join(home, ".handbeam/extensions")
        ...
        case File.read(path) do
          {:ok, content} ->
            case Manifest.from_json(content) do
              {:ok, manifest} ->
                case Handbeam.Extension.new(manifest, dir) do
                  {:ok, ext} ->
