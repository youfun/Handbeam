defmodule Handbeam.Extension.Registry do
  @moduledoc """
  Extension Registry — manages registered extensions.

  Not connected to Handbeam.Tool.Registry. Standalone Agent-based registry.
  """
  ...
  @spec list(atom() | pid()) :: [Handbeam.Extension.t()]
  def list(registry) do
    Agent.get(registry, fn state -> state.extensions |> Map.values() end)
  end

  @spec list_active(atom() | pid()) :: [Handbeam.Extension.t()]
  def list_active(registry) do
    registry |> list() |> Enum.filter(& &1.enabled)
  end

  @spec get(atom() | pid(), String.t()) :: {:ok, Handbeam.Extension.t()} | {:error, :not_found}
  ...
  @spec commit(atom() | pid(), Handbeam.Extension.t(), module() | nil) :: :ok
  ...
  @spec list_hooks_by_event(atom() | pid(), String.t()) :: [Handbeam.Extension.t()]
