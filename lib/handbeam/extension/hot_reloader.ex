defmodule Handbeam.Extension.HotReloader do
  ...
  @spec reload(keyword()) :: {:ok, [Handbeam.Extension.Diagnostic.t()]}
  def reload(opts \\ []) do
  ...
  @doc """
  If `path` is under `{project}/.handbeam/extensions`, return that project root.
  """
  ...
  def project_from_extension_path(path) when is_binary(path) do
    ...
    case Enum.split_while(parts, &(&1 != ".handbeam")) do
      {prefix, [".handbeam", "extensions" | _rest]} when prefix != [] ->
        {:ok, Path.join(prefix)}
  ...
  defp reload_project_for(path, discovered, state) do
    user_dir = Path.expand(Path.join(state.user_home, ".handbeam/extensions"))
  ...
  defp default_user_home, do: Handbeam.Home.path()

  defp ensure_watch_dirs(project, user_home) do
    [
      project && Path.join(project, ".handbeam/extensions"),
  ...
  defp reconcile_removed(previous, loaded, opts) do
    ...
    |> Enum.flat_map(fn ext ->
      ...
      _ = Handbeam.Tool.Registry.remove_owner(owner)
      _ = Handbeam.Extension.Supervisor.replace_workers(owner, [])
  ...
  defp removed_from_scanned_roots?(ext, project, user_home) do
    ...
    user_dir = Path.expand(Path.join(user_home, ".handbeam/extensions"))

    roots =
      [project && Path.expand(Path.join(project, ".handbeam/extensions")), user_dir]
      |> Enum.reject(&is_nil/1)
