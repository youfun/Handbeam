defmodule Handbeam.Tool.Extension.Beam.ProcessInfo do
  @moduledoc """
  Get detailed information about a specific BEAM process.

  Accepts a registered name (e.g. Handbeam.Repo) or PID string (e.g. "0.500.0").
  ...
  def input_schema do
    %{
      ...
      properties: %{
        process: %{
          type: "string",
          description: "Registered process name (e.g. Handbeam.Repo) or PID (e.g. \"0.500.0\")"
        }
      },
      required: ["process"]
    }
  ...
  defp resolve_pid(str) do
    cond do
      ...
      true ->
        {:error,
         "Cannot parse process reference: #{str}. Use PID (0.500.0) or registered name (Handbeam.Repo)."}
    end
  end
