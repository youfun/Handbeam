defmodule Handbeam.Tool.Extension.Beam.SessionSteer do
  @moduledoc """
  Send a message to another Handbeam session — the OTP equivalent of `tmux send-keys`.
  ...
  @impl true
  def description do
    "Send a message to another Handbeam agent session. " <>
  ...
  def execute(%{"session_id" => session_id, "message" => message} = input, _context) do
    ...
    msg =
      case role do
        :user -> Handbeam.Agent.Message.user(message)
        :assistant -> Handbeam.Agent.Message.assistant(message)
      end
    ...
    case Handbeam.PubSub.Session.whereis(session_id) do
      ...
      pid ->
        ...
        result =
          Handbeam.PubSub.Session.enqueue_candidate(session_id, msg,
