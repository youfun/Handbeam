defmodule Handbeam.Tool.Extension.Beam.Sessions do
  @moduledoc """
  List all active Handbeam agent sessions with metadata.
  ...
  @impl true
  def description do
    "List all active Handbeam agent sessions with metadata (session_id, workspace, model, status). " <>
  ...
  def execute(input, _context) do
    ...
    sessions =
      Handbeam.SessionSupervisor.which_sessions()
  ...
  defp child_to_info({:undefined, pid, :worker, [Handbeam.PubSub.Session]}) do
  ...
  defp extract_session_id(pid) do
    ...
    case Registry.keys(Handbeam.SessionRegistry, pid) do
  ...
  defp format_sessions([]) do
    "No active Handbeam sessions found."
  end
  ...
  defp format_sessions(sessions) do
    ...
    header = [
      "# Handbeam Sessions (#{total} total, #{running} running)",
      ""
    ]
