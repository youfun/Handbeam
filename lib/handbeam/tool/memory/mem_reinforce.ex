defmodule Handbeam.Tool.Memory.MemReinforce do
  @moduledoc """
  ...
  def execute(%{"id" => id}, context) do
    case Handbeam.Repo.get(Handbeam.Memory.Engram, id) do
      nil ->
  ...
  def execute(%{"query" => query}, context) do
    case Handbeam.Memory.MemoryStore.recall(query, recall_opts(context, 1)) do
      [engram | _] ->
        reinforce_accessible(engram)
  ...
  defp reinforce_accessible(engram) do
    case Handbeam.Memory.MemoryStore.reinforce(engram) do
      {:ok, reinforced} ->
