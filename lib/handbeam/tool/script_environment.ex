defmodule Handbeam.Tool.ScriptEnvironment do
  @moduledoc """
  ...
  @capabilities [
    ...
    {:json, [{Handbeam.JSON, :encode!, 1}, {Handbeam.JSON, :decode!, 1}],
     "JSON: Handbeam.JSON.encode!/1 and Handbeam.JSON.decode!/1 use the bundled OTP JSON implementation. Encode maps/lists before writing JSON files."},
    {:csv,
     [
    ...
    {:http, [{Req, :get, 2}],
     "HTTPS: Req.get(url, receive_timeout: 15_000, retry: false) returns {:ok, response} or {:error, reason}. Check response.status. JSON response.body may already be a map/list: do not blindly decode it or File.write! it; use Handbeam.JSON.encode!/1 to save JSON. Keep TLS verification enabled; send credentials only when authorized."},
    {:hash, [{:crypto, :hash, 2}],
  ...
  def snapshot do
    %{
      ...
      mix?: mix_present?(),
      hex?: hex_present?(),
      http_configured?: Handbeam.Host.get(:script_http) == :platform_dns_ca
    }
  end
  ...
  def describe(info) do
    ...
    mix_install =
      if info.mix? do
        "Mix.install/2 is supported in this script for host-compatible pure Elixir/Erlang Hex packages. Call Mix.install at the top of the .exs before using those modules. Do not call mix deps.get or an external elixir command."
      else
        "Mix and Hex are not loaded in this runtime, so Mix.install is unavailable. Do not call Mix.install, mix deps.get or an external elixir command; reuse bundled dependencies."
      end

    """
    Embedded Elixir #{info.elixir}, OTP #{info.otp}, architecture #{info.architecture}.
    Runtime modules: Mix #{presence(info.mix?)}, Hex #{presence(info.hex?)}. #{mix_install} Missing capabilities must be reported, not guessed.
  ...
  defp mix_present? do
    case Handbeam.Workspace.MixToolchain.info() do
      {:ok, info} -> info.mix?
      {:error, _} -> Code.ensure_loaded?(Mix)
    end
  end
  ...
  defp hex_present? do
    case Handbeam.Workspace.MixToolchain.info() do
      {:ok, info} -> info.hex?
      {:error, _} -> Code.ensure_loaded?(Hex)
    end
  end
