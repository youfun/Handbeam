defmodule Handbeam.Platform.ShellResolver do
  @moduledoc """
  ...
  4. Application env `:handbeam, :shell_path`
  ...
  defp read_shell_path_from_workspace(workspace_path) do
    with {:ok, settings} <- Handbeam.WorkspaceSettings.load(workspace_path) do
  ...
  defp read_shell_path_from_user do
    with {:ok, settings} <- Handbeam.Settings.load_global() do
      get_in(settings, ["tools", "bash", "shellPath"])
    end
  end

  defp resolve_app_env do
    case Application.get_env(:handbeam, :shell_path) do
      nil -> nil
