defmodule Handbeam.Agent.Auth.Storage do
  @moduledoc """
  Provider credential store backed by `~/.handbeam/auth.json`.
  ...
  @default_path "~/.handbeam/auth.json"
  @path_env "HANDBEAM_AUTH_FILE"
    ...
    case Keyword.get(opts, :auth_path) do
      ...
      _ ->
        case System.get_env(@path_env) do
          ...
          _ -> Handbeam.Home.expand(@default_path)
        end
    end
  end
  ...
  defp write_file(path, data) do
    ...
    with :ok <- File.mkdir_p(dir),
         ...
         :ok <- File.write(tmp_path, Handbeam.JSON.encode!(data, pretty: true)),
