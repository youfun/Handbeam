defmodule Handbeam.Agent.ExtensionBridge do
  @moduledoc """
  ...
  - Starts `child_spec/1` workers under `Handbeam.Extension.Supervisor`
  - Validates that declared tools have corresponding compiled modules

  Extension tools must:
  1. Implement `Handbeam.Agent.Tool` behaviour
  ...
  def load_and_integrate(opts \\ []) do
    ...
    user_home = Keyword.get(opts, :user_home, Handbeam.Home.path())
  ...
  @spec prepare(Handbeam.Extension.t()) :: {:ok, map()} | {:error, [Diagnostic.t()]}
  def prepare(ext) do
    if ext.enabled do
  ...
  """
  @spec from_extension(Handbeam.Extension.t()) :: {:ok, [Diagnostic.t()]}
  @spec from_extension(Handbeam.Extension.t(), keyword()) :: {:ok, [Diagnostic.t()]}
  ...
  @doc """
  Register a single extension tool module into Handbeam.Tool.Registry.

  The module must implement `Handbeam.Agent.Tool` behaviour and its `name/0`
  ...
  def register_extension_tool(mod, opts \\ []) when is_atom(mod) do
    with :ok <- validate_extension_tool(mod) do
      Handbeam.Tool.Registry.register(mod, opts)
    end
  end
  ...
  defp validate_tool_behaviour(mod) do
    # An extension tool must implement all callbacks of Handbeam.Agent.Tool
    required_callbacks = [
      {:name, 0},
      {:description, 0},
      {:input_schema, 0},
