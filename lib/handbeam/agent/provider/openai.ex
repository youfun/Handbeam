defmodule Handbeam.Agent.Provider.OpenAI do
  ...
  @impl true
  @spec stream([Message.t()], [Handbeam.Agent.Provider.tool_def()], config(), (String.t() -> :ok)) ::
          {:ok, Handbeam.Agent.Provider.completion_response()} | {:error, term()}
  ...
  defp build_request_body(messages, tool_defs, config) do
    ...
    body =
      ...
      |> maybe_put_optional_request_field(
        "reasoning",
        Map.get(config, :reasoning) |> Handbeam.Agent.Provider.stringify_keys()
      )
  ...
  defp maybe_append_built_in_tool(tools, type, config) when is_map(config) do
    [Map.put(Handbeam.Agent.Provider.stringify_keys(config), "type", type) | tools]
  end
  ...
  defp normalize_built_in_tool(%{"type" => _type} = tool),
    do: Handbeam.Agent.Provider.stringify_keys(tool)
  ...
  def format_tool_def(%{name: name, description: desc, input_schema: schema}) do
    %{
      ...
      "name" => name,
      "description" => desc,
      "parameters" => Handbeam.Agent.Provider.stringify_keys(schema)
    }
  end
  ...
  defp decode_function_call_arguments(%{"name" => name} = call) do
    ...
    case args do
      ...
      encoded when is_binary(encoded) ->
        case Handbeam.JSON.decode(encoded) do
          {:ok, input} -> {:ok, input}
