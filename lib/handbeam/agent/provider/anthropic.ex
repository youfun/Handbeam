defmodule Handbeam.Agent.Provider.Anthropic do
  @moduledoc """
  ...
  ## Example
      ...
      Handbeam.Agent.run("What is Elixir?",
        provider: {Handbeam.Agent.Provider.Anthropic,
  ...
  defp handle_sse_event(acc, "content_block_stop", %{"index" => index}) do
    with json_str when is_binary(json_str) <- Map.get(acc.input_json_buffers, index),
         {:ok, input} <- Handbeam.JSON.decode(json_str) do
  ...
  defp format_tool_def(%{name: name, description: desc, input_schema: schema} = def_map) do
    base = %{
      "name" => name,
      "description" => desc,
      "input_schema" => Handbeam.Agent.Provider.stringify_keys(schema)
    }
  ...
  # ── Config normalization (Handbeam adapter) ─────────────────────────────
