defmodule Handbeam.Agent.ModelConfig do
  @moduledoc """
  Global model configuration from `~/.handbeam/models.json`.

  Reads one shared model configuration file (like pi agent) to produce a provider
  config. Workspace-level restrictions live in `.handbeam/settings.jsonc`, not in
  ...
  ## Priority (highest wins)
      ...
      3. Handbeam built-in defaults (StepFun Step Plan v1)
  ...
  ## `apiKey` resolution
    ...
    - `"authType": "oauth"` → resolve/refresh from `~/.handbeam/auth.json`
  ...
  Lookup order:
    ...
    1. `HANDBEAM_MODELS_FILE` if set
    2. `~/.handbeam/models.json`
      3. Handbeam built-in defaults (StepFun Step Plan v1)
  ...
  defp config_paths(working_directory) do
    case System.get_env(@config_path_env) do
      path when is_binary(path) and path != "" ->
        [Handbeam.Home.expand(path)]

      _ ->
        _ = working_directory
        [Handbeam.Home.expand(@global_config_path)]
    end
  end
  ...
  def workspace_policy_path(workspace_root) do
    Handbeam.WorkspaceSettings.path(workspace_root)
  end
  ...
  def load_workspace_policy(workspace_root) do
    Handbeam.WorkspaceSettings.models_policy(workspace_root)
  end
  ...
  defp global_config_path do
    case System.get_env(@config_path_env) do
      ...
      _ ->
        case Application.get_env(:handbeam, :models_file) do
          path when is_binary(path) and path != "" -> Handbeam.Home.expand(path)
          _ -> Handbeam.Home.expand(@global_config_path)
        end
    end
  end
  ...
  Respects `HANDBEAM_MODELS_FILE` env var override.
  """
  ...
  (`HANDBEAM_MODELS_FILE` or `~/.handbeam/models.json`) and never applies env
  ...
  defp initial_config_body do
    case System.get_env("HANDBEAM_MODELS_SEED") do
      ...
      _ ->
        {:ok, Handbeam.JSON.encode!(@default_config, pretty: true)}
    end
  end
  ...
  def write_config(config) when is_map(config) do
    with :ok <- validate_config(config) do
      ...
      case atomic_write(path, Handbeam.JSON.encode!(config, pretty: true)) do
        :ok ->
  ...
  defp broadcast_models_updated do
    Phoenix.PubSub.broadcast(Handbeam.PubSub, "models:updated", {:models_updated})
  end
