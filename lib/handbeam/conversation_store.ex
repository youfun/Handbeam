defmodule Handbeam.ConversationStore do
  ...
  **Breaking change:** previously returned `items/<id>.json` (a file);
  ...
  Pin state is metadata. It does not rewrite the transcript. A later full
  upsert keeps the stored pin even if the caller omitted `pinned_at`.
  """
  @spec pin(String.t()) :: {:ok, map()} | {:error, :not_found | term()}
  def pin(id) when is_binary(id), do: update_meta(id, pinned_at: now_iso8601())

  @doc "Remove a conversation pin. Already unpinned conversations stay unpinned."
  @spec unpin(String.t()) :: {:ok, map()} | {:error, :not_found | term()}
  def unpin(id) when is_binary(id), do: update_meta(id, pinned_at: nil)

  @doc "True when `pinned_at` is a non-empty timestamp."
  @spec pinned_conversation?(map()) :: boolean()
  def pinned_conversation?(conversation) when is_map(conversation) do
    case pin_value(conversation) do
      value when is_binary(value) and value != "" -> true
      _ -> false
    end
  end
      ...
      {:ok, existing} ->
        {:ok,
         Map.merge(
           ...
           Map.take(
             existing,
             ~w(visibility parent_conversation_id parent_run_id parent_tool_call_id delegated_usage pinned_at)
           )
         )}
  ...
  defp normalize_conversation(conversation) do
    ...
    %{
      ...
      "pinned_at" => string_value(conversation, "pinned_at"),
