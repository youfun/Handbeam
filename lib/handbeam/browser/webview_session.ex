defmodule Handbeam.Browser.WebViewSession do
  @moduledoc """
  ...
  alias Handbeam.Browser.{Display, Engine, Scripts}
  ...
  def init(opts) do
    ...
    if is_binary(conversation_id) do
      {:ok, _} =
        Registry.register(
          Handbeam.Browser.WebViewRegistry,
  ...
  defp overlay_command(op, state) do
    Handbeam.NativeDisplay.command(%{
      op: op,
      owner: :browser,
      id: state.session_id,
      url: state.url,
  ...
  defp via(session_id) do
    {:via, Registry, {Handbeam.Browser.WebViewRegistry, session_id}}
  end
end
