defmodule Handbeam.Runtime.AndroidNotify do
  @moduledoc """
  Android host adapter for run-lifecycle notifications.

  The host injects its notifier via `Application.put_env(:handbeam, :notifier, ...)`
  (same shape as `:android_intent`): either a module exporting
  `app_visible/0`, `update_running/1`, `show_ended/1` (the `:handbeam_notify` NIF
  ...
  defp notifier_call(fun, args) do
    case Application.get_env(:handbeam, :notifier) do
