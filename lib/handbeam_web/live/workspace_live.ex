defmodule HandbeamWeb.WorkspaceLive do
  ...
  def handle_event("open_conversation_context_menu", %{"id" => conv_id}, socket) do
    {:noreply, assign(socket, :conversation_menu_id, conv_id)}
  end
  ...
  def handle_event("toggle_pin_conversation", %{"id" => conv_id}, socket) do
    socket = assign(socket, :conversation_menu_id, nil)

    socket =
      case Handbeam.ConversationStore.get(conv_id, include_timeline?: false) do
        {:ok, conv} ->
          if Handbeam.ConversationStore.internal?(conv) do
            socket
          else
            result =
              if Handbeam.ConversationStore.pinned_conversation?(conv),
                do: Handbeam.ConversationStore.unpin(conv_id),
                else: Handbeam.ConversationStore.pin(conv_id)

            case result do
              {:ok, _} ->
                ConversationSwitching.refresh_conversation_in_sidebar(socket, conv_id)
