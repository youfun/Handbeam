defmodule HandbeamWeb.SettingsLive do
  @moduledoc """
  ...
  alias HandbeamWeb.Live.Settings.ModelAIPanel
  ...
  defp subscribe_to_runtime_tasks(socket) do
    if connected?(socket) do
      ...
      Handbeam.Runtime.TaskTracker.viewing(self(), nil)
      assign_runtime_banner(socket, Handbeam.Runtime.TaskTracker.snapshot())
    else
      socket
    end
  end
  ...
  defp save_workspace_model_policy(workspace_root, policy) do
    with {:ok, settings} <- Handbeam.WorkspaceSettings.load(workspace_root),
         :ok <- File.mkdir_p(Path.dirname(Handbeam.WorkspaceSettings.path(workspace_root))) do
      settings
      |> Map.put("models", workspace_model_policy_to_json(policy))
      |> then(
        &File.write(
          Handbeam.WorkspaceSettings.path(workspace_root),
          Handbeam.JSON.encode!(&1, pretty: true)
        )
      )
      |> case do
        :ok -> :ok
