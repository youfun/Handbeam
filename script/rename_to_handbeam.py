#!/usr/bin/env python3
"""Rename Rune → Handbeam across this checkout.
...
GitHub remote `github.com/youfun/sigil` (rename the repo separately).
  ...
  python3 script/rename_to_handbeam.py --dry-run
  python3 script/rename_to_handbeam.py --apply
"""
...
SKIP_RELATIVE = {
    ...
    Path("script/rename_to_handbeam.py"),
}

# Longest-first. Bare "rune" is handled by RUNE_WORD so we do not smash
# already-renamed `:handbeam` (that token does not contain `rune`).
REPLACEMENTS: list[tuple[str, str]] = [
    ("RuneWorkspaceProbe", "HandbeamWorkspaceProbe"),
    ("RuneMixProbe", "HandbeamMixProbe"),
    ("RuneProbe", "HandbeamProbe"),
    ("RuneBridge", "HandbeamBridge"),
    ("RuneRender", "HandbeamRender"),
    ("RuneTextField", "HandbeamTextField"),
    ("RuneWeb", "HandbeamWeb"),
    ("Mix.Tasks.Rune", "Mix.Tasks.Handbeam"),
    ("RUNE_PROBE", "HANDBEAM_PROBE"),
    ("RUNE_", "HANDBEAM_"),
    ("rune_", "handbeam_"),
    ("librune", "libhandbeam"),
    ("RUNE-", "HANDBEAM-"),
...
RUNE_WORD = re.compile(r"(?<![A-Za-z0-9_])Rune(?![A-Za-z0-9_])")
RUNE_CAPS = re.compile(r"(?<![A-Za-z0-9_])RUNE(?![A-Za-z0-9_])")
RUNE_LOWER = re.compile(r"(?<![A-Za-z0-9_])rune(?![A-Za-z0-9_])")
...
def rewrite_text(text: str) -> str:
    ...
    text = RUNE_WORD.sub("Handbeam", text)
    text = RUNE_CAPS.sub("HANDBEAM", text)
    text = RUNE_LOWER.sub("handbeam", text)
...
def rewrite_name(name: str) -> str:
    ...
    if name == "rune" or stem == "rune":
        return "handbeam" + suffix
    return updated
