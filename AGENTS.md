...
- Handbeam 自己维护的业务代码统一通过 `Handbeam.JSON` 编解码 JSON；不要直接调用 `Jason` 或 OTP `:json`。
- `Handbeam.JSON` 是业务边界，负责 `nil`、atom、tuple、日期和 struct 等 Elixir 值的规范化，并由 OTP `:json` 完成实际编解码。
- 第三方依赖内部继续使用 Jason 不属于迁移范围；不要修改 `deps/` 或为隐藏传递依赖而包装第三方代码。
